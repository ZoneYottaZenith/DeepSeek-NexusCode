// Client for the nexuscode-accounts API (id.nexuscode.io). Cookie-based session,
// so every call sends credentials; the API base is build-time configurable.
const API = (import.meta.env.PUBLIC_ACCOUNTS_API || "https://id.nexuscode.io").replace(/\/$/, "");
...
// under nexuscode.io, so a subdomain (e.g. crash.nexuscode.io) can return here
...
function safeNext(next) {
  ...
  try {
    ...
    if (u.protocol === "https:" && (u.host === "nexuscode.io" || u.host.endsWith(".nexuscode.io"))) return next;
  } catch {}
  return null;
}
