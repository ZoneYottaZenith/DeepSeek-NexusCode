// NexusCode Community client. Renders the forum from the forum.nexuscode.io API and
// gates posting on the shared id.nexuscode.io session (cookie sent cross-subdomain).
// Bilingual like the rest of the site: static labels via .l-en/.l-zh spans that the
// shared `nexuscode-lang` choice toggles; plain-text strings pick the current lang.
const FORUM = (import.meta.env.PUBLIC_FORUM_API || "https://forum.nexuscode.io").replace(/\/$/, "");
const ACCOUNTS = (import.meta.env.PUBLIC_ACCOUNTS_API || "https://id.nexuscode.io").replace(/\/$/, "");
...
function setLang(l) {
  ...
  try { localStorage.setItem("nexuscode-lang", l); } catch {}
  applyLangText();
}
function initLang() {
  let saved = "";
  try { saved = localStorage.getItem("nexuscode-lang") || ""; } catch {}
...
const CATDESC = {
  ...
  show: ["Built something with NexusCode? Show the community.", "用 NexusCode 做了东西？来给社区看看。"],
...
async function renderThread() {
  ...
  document.title = `${topic.title} — ${t("NexusCode Community", "NexusCode 社区")}`;
  ...
  if (!account) {
    zone.innerHTML = `<div class="composer"><div class="gate"><p>${L("Sign in with your NexusCode account to reply.", "用你的 NexusCode 账号登录后回复。")}</p><a class="btn btn-primary" href="${loginUrl()}">${L("Sign in", "登录")}</a></div></div>`;
    return;
  }
  zone.innerHTML = `
