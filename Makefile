...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/nexuscode$(GOEXE) ./cmd/nexuscode
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/nexuscode-plugin-example$(GOEXE) ./cmd/nexuscode-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/nexuscode-$$os-$$arch$$ext ./cmd/nexuscode; \
	done
