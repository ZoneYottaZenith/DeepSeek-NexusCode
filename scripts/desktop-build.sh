...
#   macOS:   NexusCode-darwin-<arch>.zip                  (ditto archive; updater channel)
#            NexusCode-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: NexusCode-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            NexusCode-windows-<arch>.zip                 (portable human download)
#   Linux:   NexusCode-linux-<arch>.tar.gz                (bare binary; updater channel)
#            NexusCode-linux-<arch>.deb                   (Debian/Ubuntu package; human download)
#
...
APPNAME="NexusCode"            # wails.json productName -> NexusCode.app
BINNAME="nexuscode-desktop"    # wails.json outputfilename -> linux binary name
...
UPDATE_HELPER="nexuscode-update-helper.exe"
if [ "$os" = windows ]; then
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (nexuscode-desktop.app); repackage
	# it as NexusCode.app for a clean user-facing name.
	staging=$(mktemp -d)
	app="$staging/${APPNAME}.app"
	cp -R "build/bin/nexuscode-desktop.app" "$app"
