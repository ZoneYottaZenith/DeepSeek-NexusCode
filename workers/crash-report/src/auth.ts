// Dashboard authorization. Identity comes from id.nexuscode.io (the shared
...
// The id.nexuscode.io session cookie is scoped to `.nexuscode.io`, so the browser
...
function idOrigin(env: Env): string {
  return (env.ID_ORIGIN ?? "https://id.nexuscode.io").replace(/\/$/, "");
}

function appOrigin(env: Env): string {
  return (env.APP_ORIGIN ?? "https://nexuscode.io").replace(/\/$/, "");
}
...
async function identityCacheKey(token: string): Promise<Request> {
  ...
  return new Request(`https://id-cache.nexuscode.io/${hex}`);
}
...
// Ends the shared id.nexuscode.io session (best-effort) and returns a Set-Cookie
...
export async function sharedLogout(request: Request, env: Env): Promise<string> {
  ...
  return `${SHARED_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Domain=.nexuscode.io; Max-Age=0`;
}
