...
export const requireAuth: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  if (!user) throw new ApiError(401, "unauthorized", "Sign in at id.nexuscode.io to publish.");
  c.set("user", user);
  await next();
};
...
export const requireAdmin: MiddlewareHandler<AppEnv> = async (c, next) => {
  const user = await fetchAccountUser(c).catch(() => null);
  if (!user) throw new ApiError(401, "unauthorized", "Sign in at id.nexuscode.io.");
