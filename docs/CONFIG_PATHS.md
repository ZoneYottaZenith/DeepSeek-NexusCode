...
Starting with **NexusCode v1.8.1**, NexusCode uses one user-facing home directory
for global configuration and user-owned state. CLI and desktop share this
location.

## NexusCode Home

| Platform | NexusCode home |
| --- | --- |
| macOS | `~/.nexuscode` |
| Linux | `~/.nexuscode` |
| Windows | `%APPDATA%\nexuscode` |

Set `NEXUSCODE_HOME` to override NexusCode home for tests, CI, or portable
installations. Normal users should not need it.

When `NEXUSCODE_HOME` is set, the runtime is fully self-contained: all
...
Advanced test and portable setups may set `NEXUSCODE_STATE_HOME` to move runtime
state such as sessions, archives, and memory. It does not move global config or
provider credentials: those remain under `NEXUSCODE_HOME`. If an older build wrote
provider keys to `NEXUSCODE_STATE_HOME/.env`, NexusCode imports those keys
non-destructively when `<NexusCode home>/.env` is missing them.

## What Lives There

| Data | Path |
| --- | --- |
| Global config | `<NexusCode home>/config.toml` |
| Global provider credentials | `<NexusCode home>/.env` |
| Legacy credentials import source | `<NexusCode home>/credentials` |
| Global slash commands | `<NexusCode home>/commands/` |
| Global skills | `<NexusCode home>/skills/` |
| Global hooks | `<NexusCode home>/settings.json` |
| Hook trust store | `<NexusCode home>/trust.json` |
| Sessions | `<state root>/sessions/` |
| Archives | `<state root>/archive/` |
| Memory | `<state root>/memory/` and `<state root>/projects/` |

`<state root>` defaults to `<NexusCode home>`. It only differs when
`NEXUSCODE_STATE_HOME` is set.

The global user config is named `config.toml`. Project-local config files keep
the name `nexuscode.toml`. If someone says "global nexuscode.toml", they usually
mean `<NexusCode home>/config.toml`.

## Global `config.toml`

`<NexusCode home>/config.toml` stores non-secret configuration shared by the CLI
and desktop app. It may contain the same provider, plugin, UI, desktop, tool,
skill, sandbox, bot, and agent settings that NexusCode renders into user config.
...
When a custom provider is added from the desktop settings or `nexuscode setup`,
NexusCode stores a generated `api_key_env` in `config.toml` and writes the secret
...
NexusCode derives the default from the provider name. Names that normalize to
...
NexusCode sends chat requests to `base_url + "/chat/completions"` and probes model
discovery candidates such as `/models` and `/v1/models`. If a gateway gives you a
complete chat request URL, set `chat_url`; NexusCode will use it directly and will
...
`<NexusCode home>/.env` is the single runtime source for provider API keys saved
by NexusCode. The setup wizard, desktop settings, CLI missing-key prompts, and
...
- multiline values are rejected by NexusCode writes;
- keys must use shell-style names such as `DEEPSEEK_API_KEY`;
- `# nexuscode-cleared KEY` comments are non-secret tombstones written after a key
  is deleted so legacy stores do not silently re-import it;
- NexusCode writes this file with restricted permissions where the OS supports
  them.

For provider requests, NexusCode resolves only this global `.env`. Project `.env`
...
into the process environment, and NexusCode control variables such as
`NEXUSCODE_HOME`, `NEXUSCODE_STATE_HOME`, and `XDG_CONFIG_HOME` are ignored there.

Caches remain in the OS cache directory, for example
`~/Library/Caches/nexuscode` on macOS, `$XDG_CACHE_HOME/nexuscode` or
`~/.cache/nexuscode` on Linux, and `%LOCALAPPDATA%\nexuscode\cache` on Windows.
Set `NEXUSCODE_CACHE_HOME` to override the cache root. When `NEXUSCODE_HOME` is
set, the cache is placed under `$NEXUSCODE_HOME/cache` (unless
`NEXUSCODE_CACHE_HOME` is also set, which takes precedence).
...
```text
command-line flags
> project ./nexuscode.toml
> global <NexusCode home>/config.toml
...
```text
macOS/Linux: ~/.nexuscode/config.toml
Windows:     %APPDATA%\nexuscode\config.toml
```

## Legacy Migration

Starting with **v1.8.1**, NexusCode automatically checks legacy locations on
startup before the first config load. Migration is synchronous, one-time, and
non-destructive: old files are copied or converted to NexusCode home and left
untouched.

Legacy config sources include:

```text
~/Library/Application Support/nexuscode/config.toml
~/.config/nexuscode/config.toml
~/.nexuscode/nexuscode.toml
~/.nexuscode/config.json
```

Legacy credentials, memory files, and sessions are also imported into NexusCode
home when the new destination does not already exist. Legacy provider keys are
copied into `<NexusCode home>/.env` only when that file does not already contain
...
Starting in **v1.9.1**, NexusCode also backfills MCP servers from known legacy
paths, legacy `config.json`, desktop-registered projects, and restored tab
projects into the global `<NexusCode home>/config.toml`. Existing global
...
If NexusCode has already created the new home directory but some legacy data was
...
```text
/migrate --from "D:\OldNexusCode"
```

The explicit form imports sessions only. The path may be the old install
directory, a `.nexuscode`/data directory, or the `sessions` directory itself;
NexusCode checks the common layouts below that root and uses a source-specific
...
existing `<NexusCode home>/config.toml`; if the new config already exists, copy
...
- `/migrate` is available only in Go-based NexusCode builds that include the
  command. If NexusCode reports `unknown command`, upgrade first and rerun it.
