...
NexusCode already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.nexuscode/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing NexusCode gates still apply.
...
```text
.nexuscode/autoresearch/<task-id>/
  state/
    task_spec.json
    progress.json
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.nexuscode/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.nexuscode/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.nexuscode/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .nexuscode/autoresearch/<task-id>/`
...
- explicit `.nexuscode/autoresearch/<task-id>/` resumes existing state
...
No existing `.nexuscode/autoresearch` task should be deleted or rewritten without
