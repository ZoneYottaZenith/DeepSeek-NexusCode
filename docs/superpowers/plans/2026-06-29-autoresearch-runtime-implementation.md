...
- [ ] Write failing tests that prove `CreateTask` creates `.nexuscode/autoresearch/<task-id>/state` and `logs`, writes `task_spec.json`, `progress.json`, empty JSONL files, and validates required fields.
