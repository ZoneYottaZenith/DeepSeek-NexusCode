# Migrating to NexusCode 1.0 (the Go rewrite)

NexusCode 1.0 is a **ground-up rewrite in Go**. It is a new codebase, not an
incremental upgrade of the `0.x` TypeScript releases. This guide explains what
changed and how to move over.

## TL;DR

| | Legacy (v1) | NexusCode 1.0+ (v2) |
|---|---|---|
| Language | TypeScript / Node | Go |
| Branch | [`v1`](https://github.com/ZoneYottaZenith/DeepSeek-NexusCode/tree/v1) (maintenance only) | `main-v2` (default, active) |
| Versions | `0.x` (up to v0.54.x) | `1.0.0`+ |
| Install | `npm i -g nexuscode@0.53.2` (pin a `0.x` version) | `npm i -g nexuscode` — `latest` points at the current `1.x` stable; or a release archive / `go build` |
...
**`npm i -g nexuscode` installs the current `1.x` stable.** npm's `latest` tag
...
```sh
npm i -g nexuscode          # current 1.x stable
npm i -g nexuscode@next     # release candidate, when one is ahead of stable
npm i -g nexuscode@0.53.2   # pin the legacy TS build
```

Prebuilt archives (`nexuscode-<os>-<arch>.tar.gz` / `.zip`) and the desktop
...
```sh
git clone https://github.com/ZoneYottaZenith/DeepSeek-NexusCode   # default: main-v2 (Go)
cd DeepSeek-NexusCode && make build                        # -> bin/nexuscode(.exe)
```

## Configuration

| Legacy | NexusCode 1.0 |
|---|---|
| TS config files | `nexuscode.toml` (project) / `config.toml` in NexusCode home (`~/.nexuscode/` on macOS/Linux; `%AppData%\nexuscode\` on Windows) from v1.8.1 — see `nexuscode.example.toml` and [Configuration paths](./CONFIG_PATHS.md) |
| env / API keys | Provider config keeps `api_key_env`; saved key values live in NexusCode home `.env` (`DEEPSEEK_API_KEY`, `MIMO_API_KEY`, …) |
| project memory | `NEXUSCODE.md` (+ auto-memory), Claude-Code-compatible |
| MCP servers | `[[plugins]]` in `nexuscode.toml`, or a Claude-Code `.mcp.json` (read as-is) |

On first launch, v1.8.1+ runs a one-time, **non-destructive** import: it reads
legacy config from `~/Library/Application Support/nexuscode/config.toml`,
`~/.config/nexuscode/config.toml`, `~/.nexuscode/nexuscode.toml`, or v0.x
`~/.nexuscode/config.json` (API key, base URL, language, MCP servers), migrates
legacy credentials into `<NexusCode home>/.env` when a key is missing there, and
imports past sessions from legacy session directories. Old files are left
untouched, and NexusCode prints a boot notice when it imports data. Each session lands in the
...
The command is available only in Go-based NexusCode builds that include it; if you
...
`/migrate --from "D:\OldNexusCode"` to import sessions from that explicit source.
See
...
NexusCode 1.0 supports reading and editing files in UTF-8, UTF-8 BOM, UTF-16
...
Questions? Open a [Discussion](https://github.com/ZoneYottaZenith/DeepSeek-NexusCode/discussions).
