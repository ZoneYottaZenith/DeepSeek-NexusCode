# NexusCode Engineering Spec

> NexusCode is a coding agent: a thin harness driving multiple models, with **all
...
├── cmd/nexuscode-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .nexuscode/commands/*.md
...
- `cmd/nexuscode-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. NexusCode manages this with
...
- The dropped originals are archived under the user config dir
  (`nexuscode/archive/<timestamp>.jsonl`; see §5 for its per-OS location), one
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  non-interactive run
  (`nexuscode run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
- **Collaboration mode is separate from tool approval.** The desktop composer
  ...
  `.nexuscode/autoresearch/YYYYMMDD-HHMMSS-slug/` and keeps dynamic run state out
  of `NEXUSCODE.md`, `AGENTS.md`, project memory, tool schemas, and the
...
Out of the box (`mode = "ask"`, no rules) `nexuscode run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `nexuscode` now prompts before
...
- **Custom commands** are Markdown files under `.nexuscode/commands/` (project) and
  the user config dir, e.g. `~/.nexuscode/commands/` on macOS/Linux; the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — NexusCode
...
Resolution order: **flag > project `./nexuscode.toml` > the user config file
> built-in defaults**. Starting with **NexusCode v1.8.1**, the user config lives
at `~/.nexuscode/config.toml` on macOS/Linux and
...
without importing provider keys or NexusCode control variables. Step-limit
preferences belong in the user config.
Project `nexuscode.toml` does not override `agent.max_steps` or
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $NEXUSCODE_LANG
...
[agent]
system_prompt = "You are NexusCode, a coding agent..."  # or system_prompt_file = "..."
...
temperature       = 0.0
memory_compiler = { enabled = true, verbosity = "observe" }   # user/global only; observe|compact; CLI: nexuscode config memory-v5 off|observe|compact|on|status
...
`nexuscode setup` writes this default config so the CLI is usable out of the box.
...
`[serve]` controls the HTTP browser frontend used by `nexuscode serve`. The
...
`[[plugins]]`; on a name collision `nexuscode.toml` wins (it is the more explicit,
NexusCode-specific source). This lets a server already configured for Claude work in
NexusCode unchanged.
...
`multi_edit`, `move_file`) to `workspace_root` (default cwd), the NexusCode user
...
`nexuscode doctor` flags the ignored setting) — because the native backend
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o nexuscode ./cmd/nexuscode`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  offer an explicit, permission-gated unconfined rerun (in `nexuscode run`, the
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `nexuscode run`.
