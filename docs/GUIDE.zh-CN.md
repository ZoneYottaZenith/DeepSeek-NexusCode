...
优先级：**flag > `./nexuscode.toml` > 用户配置文件 > 内置默认值**。从
**NexusCode v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.nexuscode/config.toml`，Windows 为 `%AppData%\nexuscode\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./nexuscode.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
NexusCode 全局 `<NexusCode home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 NexusCode 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
插件包可通过 `hooks/session-start-codex` 或插件根目录 `CLAUDE.md` 提供该启动上下文；Claude 风格 `.claude/settings.json` command hooks 也会按同名事件映射到 NexusCode hooks。

```toml
default_model = "deepseek-flash"   # 执行器；设 [agent].planner_model 可加规划器
# language    = "zh"               # 界面语言；为空则按 $LANG / $NEXUSCODE_LANG 自动检测
...
# password_hash = ""           # 用 nexuscode serve --hash-password --password '...' 生成
...
`[agent].plan_mode_allowed_tools` 用于把 NexusCode 无法自动分类的自定义/外部工具声明为额外只读工具。
...
当计划阶段需要运行 NexusCode 尚不能自动分类、但你确认只读的 shell 查询命令时，使用
...
NexusCode 也可以提示你确认是否信任它为只读；选择持久信任会写入同一个
`[agent].plan_mode_read_only_commands` 配置。Auto/YOLO 审批不会替用户回答这个信任提示。

### 环境变量

多数日常设置应写在 `config.toml` 或前文提到的 NexusCode 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 NexusCode 之前设置。项目 `.env` 不是 NexusCode 控制变量的运行时来源。

`NEXUSCODE_MEMORY_COMPILER_LLM_CLASSIFICATION=true` 会为 Memory v5 启用可选的 LLM 任务/聊天分类器。
默认关闭，此时 NexusCode 使用本地 heuristic classifier，不会产生额外 provider 调用。开启后，分类缓存未命中时，
NexusCode 可能先通过已配置 provider 发送一个很小的分类请求，再决定用户输入是任务还是普通对话；这会增加少量延迟、
...
`nexuscode serve` 会用同一个本地 NexusCode 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
常用服务优先使用 **添加模型服务 -> 推荐预设**。NexusCode 可以预填可编辑的自定义 provider：
...
预设路径通常只需要填写服务商 API Key：真实 key 会写入 NexusCode home `.env`，
...
**API 地址** 填写服务端点。默认模式下，NexusCode 会预览并把聊天请求发送到：
...
开启 **完整 URL**。开启后 NexusCode 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
| 字段 | 作用 | 什么时候改 |
| --- | --- | --- |
| `api_key_env` | 该 provider 使用的 API key 环境变量名。桌面端保存的真实 key 会写入 NexusCode home `.env` 的同名变量；TOML 配置里只保存变量名。 | 多个 provider 需要不同 key 时改名；服务不需要 API key 时可以留空。 |
...
| 额外请求体 | 合并到聊天请求体顶层的 JSON 对象。 | 仅用于服务商专用开关，例如 `{"enable_thinking": true}`。`model`、`messages`、`tools`、`stream`、`thinking` 等核心字段仍由 NexusCode 控制，且不接受 `null` 值。 |
...
| 模型能力模式 | 指定 NexusCode 对该 provider 使用哪种 reasoning 请求协议。 | 默认用“自动识别”。只有网关被误判，或模型文档要求特定 reasoning 格式时再切换。 |
...
| 上下文窗口 | 该 provider 可保留的最大上下文 token 数。`0` 表示使用模型服务默认值。 | 模型实际上下文大小和 NexusCode 默认值或内置元数据不一致时填写。 |

模型能力模式选项：

| 选项 | 作用 |
| --- | --- |
| 自动识别（推荐） | NexusCode 根据模型能力元数据和端点自动选择请求格式。 |
...
| 选项 | 作用 |
| --- | --- |
| Auto（使用服务默认） | 不写 provider 级 `thinking` 覆盖，让 NexusCode 使用 provider/model 默认行为。 |
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，NexusCode 会为桌面端保存该绑定。
...
| 终端原生选择 | 复制 transcript 文本 | NexusCode 默认不启用鼠标报告，因此终端自己的选择/复制仍可使用。 |
...
`nexuscode` 会在 writer 调用前征求同意（普通工具为 `1` 本次 · `2` 本会话允许此范围 · `3` 总是允许此范围（保存） · `4` 拒绝；Bash 可额外选择命令前缀授权）；
其中 Bash 默认按具体命令记，也可按安全推导出的命令前缀记（如 `Bash(go test:*)`）；文件编辑类工具的本会话授权按编辑能力记，持久授权则写入 `Edit(<path>)` 文件路径规则；
`nexuscode run` 保持自主运行但仍然遵守 `deny`。
...
原生 Windows helper 使用 NexusCode 内置的 Windows sandbox backend：
...
`off`——即使显式写了 `bash = "enforce"` 也会解析为 `off`（`nexuscode doctor`
...
Windows 沙盒排障：沙盒会把 NexusCode 可执行文件自身以隐藏 helper 方式重新拉起，
...
low-integrity token 下较脆弱。运行 `nexuscode doctor` 可查看解析到的 shell、沙盒
可用性，以及项目 `nexuscode.toml` 是否固定了 `[sandbox]`（项目文件优先级高于
Settings/用户配置；沙盒配置变更需 reload session config 或新开会话才生效）。

## 插件（MCP）

NexusCode 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
`mcpServers.<server>.trusted_read_only_tools`，普通 NexusCode plugin 会写入用户级 NexusCode config。
...
各自暴露的内容。`make build` 还会产出 `bin/nexuscode-plugin-example`——一个可直接运行的
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，NexusCode 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `nexuscode.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.nexuscode/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`nexuscode.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

交互式 `nexuscode` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/memory-v5`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
是放在 `.nexuscode/commands/`（项目）或 `~/.nexuscode/commands/`（用户）下的 Markdown 文件——
`review.md` 即 `/review`，子目录构成命名空间（`git/commit.md` → `/git:commit`）。文件正文
是 prompt 模板，调用即作为一轮对话发出。

`/memory` 会同时列出记忆文档（`NEXUSCODE.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
Memory v5 在 CLI/TUI、`nexuscode serve` 和桌面端默认开启，因为这些入口共用同一套本地
controller。它会把本地、按项目隔离的执行轨迹和编译器状态写在 NexusCode home 下，并且只有
...
`nexuscode config memory-v5 off|observe|compact|on|status`。桌面端还可以在设置 → 通用 → Memory v5 中控制。
...
CLI/TUI 和 `nexuscode serve` 使用同一个 user/global 配置。项目内的 `nexuscode.toml` 不能覆盖
这个 user/global 设置。CLI 命令会更新底层配置；高级用户也可以手动编辑 NexusCode home 下的
用户配置：
...
`nexuscode run --metrics <path>` 时，JSON 还会输出内容无关的 `memory_compiler_*` 汇总字段，
...
Goal 是长期目标的统一运行机制。普通 `/goal` 继续走轻量 Goal：NexusCode 会持续推进，直到
...
`.nexuscode/autoresearch/<task-id>/` 任务目录。高级用户可以用
...
它会创建或复用项目级 `.nexuscode/autoresearch/<task-id>/` 目录。新任务默认使用
...
`nexuscode setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `nexuscode.toml` 加一行即可：
...
Planner 会看到已加载的 `NEXUSCODE.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
因此可以先检查相关文件再把计划交给执行器。写入类和流程类工具仍只给执行器使用。
`max_steps` 限制执行器；`planner_max_steps` 只限制规划器，两者都可设为 `0` 表示不限。

轮数上限请放在用户级配置。项目 `./nexuscode.toml` 不会覆盖 `max_steps` 或
`planner_max_steps`。
...
的任务会自动进入 plan mode：NexusCode 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
在 `nexuscode` 会话里用 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`nexuscode config auto-plan off|on`。Auto-plan 只认用户级设置；项目
`nexuscode.toml` 里的 `agent.auto_plan` 会被忽略。可见思考语言也采用类似形态：
会话里用 `/reasoning-language auto|zh|en`，shell/脚本里用
`nexuscode config reasoning-language auto|zh|en`。Memory v5 使用 `/memory-v5 off|observe|compact|on|status`
或 `nexuscode config memory-v5 off|observe|compact|on|status`，并且只认用户级设置。只有明确想为
