...
Inside `nexuscode`, use the slash command:
...
`nexuscode.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
nexuscode run "explain this module"
```
...
```text
./nexuscode.toml > user config.toml > built-in defaults
```
...
NexusCode adds the same small transient `<reasoning-language>` block for that
...
When set to `zh` or `en`, NexusCode always adds a small transient
