...
Resolution order: **flag > `./nexuscode.toml` > the user config file >
built-in defaults**. Starting with **NexusCode v1.8.1**, the user config lives at
`~/.nexuscode/config.toml` on macOS/Linux and
`%AppData%\nexuscode\config.toml` on Windows; see
...
NexusCode's global `<NexusCode home>/.env`, shared by CLI and desktop. Project
...
provider keys or NexusCode control variables. See
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $NEXUSCODE_LANG
...
# password_hash = ""           # bcrypt hash generated with nexuscode serve --hash-password --password '...'
# behind_proxy = false         # true only behind a trusted reverse proxy

[[plugins]]
name    = "example"
command = "nexuscode-plugin-example"
...
external tools NexusCode cannot classify itself. For MCP/plugin tools, a concrete
...
specific shell command that NexusCode cannot classify but you know is read-only,
...
command flags remain blocked while planning. In interactive plan mode, NexusCode
...
Most day-to-day settings belong in `config.toml` or the global NexusCode `.env`
described above. The variables below are process-level advanced switches; set
them before launching NexusCode. Project `.env` files are not a runtime source for
NexusCode control variables.

`NEXUSCODE_MEMORY_COMPILER_LLM_CLASSIFICATION=true` enables the optional LLM
task/chat classifier for Memory v5. By default it is disabled, and NexusCode uses
...
```bash
NEXUSCODE_MEMORY_COMPILER_LLM_CLASSIFICATION=true nexuscode
```

For development runs, prefix the command that starts the process, for example:

```bash
NEXUSCODE_MEMORY_COMPILER_LLM_CLASSIFICATION=true wails dev -forcebuild
```
...
`nexuscode serve` starts the same local engine behind a browser UI. Use it when
you want a desktop-style surface without installing the desktop app, when running
NexusCode on a remote development box through a tunnel, or when you want a
...
```bash
nexuscode serve --auth token
nexuscode serve --addr 0.0.0.0:8787 --auth token
nexuscode serve --auth password --password 'temporary-password'
```
...
```bash
nexuscode serve --hash-password --password 'strong-password'

# <NexusCode home>/config.toml
[serve]
...
NexusCode can prefill editable custom-provider entries for Kimi CN, Kimi Global,
...
usually needs only the provider API key: the key value is stored in NexusCode home
...
chat path. In this mode NexusCode previews and sends chat requests to:
...
example `https://gateway.example.com/v1/chat/completions`. NexusCode then sends
...
| Extra request body | A JSON object merged into the top-level chat request body. | Use only for provider-specific flags such as `{"enable_thinking": true}`. NexusCode still owns core fields such as `model`, `messages`, `tools`, `stream`, and `thinking`, and null values are rejected. |
...
| Model capability mode | Which reasoning request protocol NexusCode should use for this provider. | Keep **Auto-detect** unless the gateway is misdetected or the model docs require a specific reasoning format. |
...
| Context window | The maximum number of tokens this provider keeps in context. `0` means provider default. | Set it when the model's real context size differs from NexusCode's default or built-in metadata. |

Model capability mode options:

| Option | Effect |
| --- | --- |
| Auto-detect (recommended) | NexusCode chooses the request shape from model capability metadata and endpoint detection. |
...
| Option | Effect |
| --- | --- |
| Auto (provider default) | Does not write an explicit provider-level `thinking` override. NexusCode uses the provider/model default behavior. |
...
`extra_body` is merged into the chat JSON request body. NexusCode keeps core
...
`hookSpecificOutput.additionalContext`; NexusCode injects that text once into the
...
`.claude/settings.json` command hooks are also mapped to matching NexusCode hook
events.
...
new key combination, and NexusCode saves it for the desktop app. Conflicting
...
| `/mouse` | Toggles in-app mouse capture | Off hands the mouse back to your terminal, restoring its native click-drag selection and right-click context menu, at the cost of in-app drag-select, the transcript scrollbar, and wheel-scroll. Set `NEXUSCODE_DISABLE_MOUSE=1` to start every session with it off. |
...
`nexuscode` can grant Bash as an exact command or as a conservative command
...
`nexuscode run` stays autonomous but still honours `deny`.
...
The native Windows helper uses NexusCode's bundled Windows sandbox backend:
...
`off`, and `nexuscode doctor` flags the ignored setting — because the native
...
Windows sandbox troubleshooting: the sandbox relaunches the NexusCode
...
`nexuscode doctor` to see the resolved shell, sandbox availability, and whether
a project `nexuscode.toml` pins `[sandbox]` (a project file overrides
Settings/user-config edits, and sandbox changes take effect after a session
config reload or a new session).

## Plugins (MCP)

NexusCode is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
`mcpServers.<server>.trusted_read_only_tools`, while ordinary NexusCode plugins
are updated in the user's NexusCode config. Trust only side-effect-free readers;
...
exposes. `make build` also produces `bin/nexuscode-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and NexusCode
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `nexuscode.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.nexuscode/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `nexuscode.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In an interactive `nexuscode` session, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.nexuscode/commands/`
(project) or `~/.nexuscode/commands/` (user) — `review.md` becomes
...
`/memory` lists both memory documents (`NEXUSCODE.md` / `AGENTS.md`) and saved
...
Memory v5 is enabled by default across the CLI/TUI, `nexuscode serve`, and the
desktop app because they all share the same local controller. It records local,
project-scoped execution traces and compiler state under NexusCode home, then
...
interactive session, or with `nexuscode config memory-v5 off|observe|compact|on|status`
from a shell/script.
...
CLI/TUI and `nexuscode serve` use the same user/global config. Project
`nexuscode.toml` files cannot override this user/global setting. The CLI command
...
aggregate metrics upload pipeline. When `nexuscode run --metrics <path>` is used,
...
objectives stay lightweight: NexusCode keeps working until the goal is complete,
...
`.nexuscode/autoresearch/<task-id>/` directory. Advanced users can force it with
...
`.nexuscode/autoresearch/<task-id>/` directory. For new tasks, the default id
shape is `YYYYMMDD-HHMMSS-slug`, such as `20260618-224530-cache-audit`; NexusCode
...
broad requirement. Dynamic run state stays in `.nexuscode/autoresearch/...`, not
in `NEXUSCODE.md`, `AGENTS.md`, project memory, tool schemas, or the cache-stable
...
Embed `@` references in a message and NexusCode resolves them before sending, as
...
`nexuscode setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `NEXUSCODE.md` / `AGENTS.md` memory and a small read-only
...
Keep step-limit preferences in the user config. Project `./nexuscode.toml` files
...
automatically: NexusCode first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` inside `nexuscode` to change the user-level setting, or
`nexuscode config auto-plan off|on` from a shell/script. Auto-plan is user-level
only; `agent.auto_plan` in a project `nexuscode.toml` is ignored. The visible
reasoning language uses a similar shape: `/reasoning-language auto|zh|en` in the
session, or `nexuscode config reasoning-language auto|zh|en` in a shell/script.
Memory v5 uses `/memory-v5 off|observe|compact|on|status` or
`nexuscode config memory-v5 off|observe|compact|on|status` and is user-level only. Pass `--local`
