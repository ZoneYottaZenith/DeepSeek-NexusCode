...
NexusCode 插件包把 skills、hooks 和 MCP servers 组织成一个可安装单元。

## CLI 模式

在终端里使用 `nexuscode plugin` 安装和管理插件包。插件包当前按全局范围安装，
写入 NexusCode home 目录。
...
- 本地目录，目录内需要包含 `nexuscode-plugin.json`、
...
- `--name <name>` 或 `--name=<name>` 覆盖插件 manifest 里的名称，
  作为本次安装名称。
- `--link` 链接本地插件目录，而不是复制到 NexusCode 的插件存储目录。
  移动或删除该目录会导致这个链接插件失效。

如果运行 `nexuscode plugin install <source>` 时既没有 `--dry-run`，
...
已安装插件不会打开一个独立聊天界面。插件启用后，NexusCode 会把它的能力加载到普通交互会话里：
...
- **MCP servers** 会进入正常 MCP/工具流程。用户只需要描述任务，
  NexusCode 会在相关时调用插件提供的工具。

如果是在另一个终端里安装、启用、禁用或更新插件，而当前已有 `nexuscode` 会话正在运行，
...
NexusCode 原生插件在根目录声明 `nexuscode-plugin.json`：
...
相对路径都按插件根目录解析。NexusCode 安装插件时不会执行第三方安装脚本。

## Codex 与 Claude 兼容

NexusCode 也会读取 `.codex-plugin/plugin.json` 和 `.claude-plugin/plugin.json`。
NexusCode 尚未映射的 Claude 插件能力（`commands/`、`agents/`、`hooks/hooks.json`、
`.mcp.json`）会以安装警告的形式提示，而不是被静默丢弃；多插件的
`marketplace.json` 索引暂不支持——请逐个安装插件目录。
对于 Superpowers 和 Claude 风格 skill 包，NexusCode 会映射：

- `skills` 到 NexusCode skill root。Claude 清单若未声明 `skills` 字段，会回退到
  约定目录 `skills/`（或 `.claude/skills/`），与 Claude 自身的自动发现一致。
- 如果存在 `hooks/session-start-codex`，映射为 NexusCode `SessionStart` hook。
- 插件根目录的 `CLAUDE.md` 会映射为内置的 `SessionStart` 上下文 hook。
  NexusCode 会直接读取该文件，不通过 shell 命令。
- `.claude/settings.json` 里的 command hooks 会按同名事件映射到 NexusCode hooks。
  Claude 的 `matcher` 字段会映射到 NexusCode `match`；hook 命令以插件根目录作为
  `cwd` 执行；Claude `timeout` 按秒解析。

不支持的 Claude hook item type 会跳过并产生 warning。NexusCode 不会执行第三方安装脚本，
