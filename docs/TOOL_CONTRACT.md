...
This document records the provider-visible contract for NexusCode compile-time built-in tools. It is generated from the same canonical schema path used by the runtime registry.
...
In a default full-token boot, NexusCode sends the built-in tools above plus the
...
In token economy mode, NexusCode starts with the core coding/session/memory tools
