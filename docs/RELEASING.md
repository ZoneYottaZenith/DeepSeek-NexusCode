...
How NexusCode ships, who can ship what, and the canary-before-stable flow.
...
|---|---|---|
| npm | `latest` (current 1.x stable) | `next` (rc), `canary` (`npm i nexuscode@canary`) |
...
| **Ship `next` / stable** | **repo maintainer** | stable publish jobs gate on the `release` environment. |

So a maintainer can dispatch a canary anytime, but a stable release pauses in the
Actions UI until the `release` environment deployment is approved.
...
3. **Test** — testers install `nexuscode@canary` (CLI) or grab the desktop canary
...
5. **Ship stable** when the canary is clean — push the three tags:
   ...
   ```
   Each stable run **waits for the `release` environment approval** before publishing.
...
- Desktop in-app updates use R2 first, then the `crash.nexuscode.io` desktop release
