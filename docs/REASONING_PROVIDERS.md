...
NexusCode exposes a single `/effort` knob (and the per-provider `effort` /
...
Distinguishing "provider ignores the field" from a NexusCode bug starts here:
the request shape NexusCode emits is fixed per the table, so a mismatch between
the table and observed behaviour is the provider's, not NexusCode's.
