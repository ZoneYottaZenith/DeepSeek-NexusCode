# NexusCode Plugin Packages

NexusCode plugin packages bundle skills, hooks, and MCP servers behind one
installable unit.

## CLI Mode

Use `nexuscode plugin` when installing or managing plugin packages from a
terminal. Plugin packages are installed globally under the NexusCode home
directory.
...
- A local directory that contains `nexuscode-plugin.json`,
  `.codex-plugin/plugin.json`, or `.claude-plugin/plugin.json`.

Preview the install plan without writing files:

```bash
nexuscode plugin install git:github.com/obra/superpowers --dry-run
```

Install a plugin after reviewing the plan:

```bash
nexuscode plugin install git:github.com/obra/superpowers --yes
```

Install with an explicit name or replace an installed plugin with the same name:

```bash
nexuscode plugin install git:github.com/obra/superpowers --name superpowers --replace --yes
```

Use a local directory in developer mode:

```bash
nexuscode plugin install /path/to/plugin --link --replace --yes
```
...
- `--link` links a local plugin directory instead of copying it into NexusCode's
  plugin storage. Moving or deleting that directory breaks the linked plugin.

Running `nexuscode plugin install <source>` without `--dry-run` or `--yes`
...
```text
~/.nexuscode/plugin-packages.json
~/.nexuscode/plugins/<name>/
```
...
```bash
nexuscode plugin doctor superpowers
```

Enable or disable a plugin without uninstalling it:

```bash
nexuscode plugin disable superpowers
nexuscode plugin enable superpowers
```

Remove a plugin:

```bash
nexuscode plugin remove superpowers --yes
```
...
NexusCode loads its capabilities into normal interactive sessions:
...
- **MCP servers** join the normal MCP/tool flow. Ask for the task you want done;
  NexusCode can call the plugin's tools when they are relevant.

After installing, enabling, disabling, or updating a plugin from a separate
terminal while a session is already running, start a new `nexuscode` session or
...
- **Developer mode: link source folder** appears for **Local folder** installs.
  It links the selected directory instead of copying it into NexusCode's plugin
...
NexusCode plugins can declare `nexuscode-plugin.json` at the plugin root:
...
Relative paths are resolved inside the plugin root. NexusCode does not run
third-party install scripts during plugin installation.

## Codex & Claude Compatibility

NexusCode also reads Codex plugin manifests at `.codex-plugin/plugin.json` and
Claude Marketplace manifests at `.claude-plugin/plugin.json`. Claude plugin
capabilities NexusCode does not map yet (`commands/`, `agents/`,
...
as Superpowers and Claude-style skill packs, NexusCode maps:

- `skills` to NexusCode skill roots. A Claude manifest that declares no
  `skills` field falls back to the conventional `skills/` (or `.claude/skills/`)
  directory, matching Claude's own auto-discovery.
- `hooks/session-start-codex` to the NexusCode `SessionStart` hook when present.
- A plugin-root `CLAUDE.md` file to a built-in `SessionStart` context hook. The
  file is read directly by NexusCode, without spawning a shell command.
- `.claude/settings.json` command hooks to NexusCode hook events when the event
  names match. Claude's `matcher` field maps to NexusCode `match`; hook commands
  run as shell commands with the plugin root as `cwd`; Claude `timeout` values
  are interpreted as seconds.

Unsupported Claude hook item types are skipped with a warning. NexusCode does not
...
- `NEXUSCODE_PLUGIN_ROOT`
- `NEXUSCODE_PLUGIN_NAME`
- `NEXUSCODE_PLUGIN_VERSION`
- `NEXUSCODE_HOME`
- `NEXUSCODE_WORKSPACE_ROOT`
- `CLAUDE_PROJECT_DIR`
