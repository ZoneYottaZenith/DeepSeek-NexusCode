...
> WeChat, and QQ bots, how to use NexusCode from IM, and how approvals, Ask
...
After a bot is connected, you can send NexusCode messages from Feishu, Lark,
WeChat, or QQ. The desktop app or `nexuscode bot start` process handles the
model, tools, permissions, sandboxing, and local context, then sends progress
and results back to the IM channel.

Common uses:

- Ask NexusCode to inspect code, read docs, explain errors, or summarize findings.
...
- **CLI runtime**: run `nexuscode bot start` for a headless long-lived process.
...
The normal `nexuscode run` command does not automatically start the IM gateway.
Remote bot behavior is active only while the desktop bot runtime is running or
while a `nexuscode bot start` process is alive.

## Connect the four channels

Open the NexusCode desktop app and go to **Settings -> Bots**. In **Add IM Bot**,
...
CommonMark formatting. If a card is too large for the platform limit, NexusCode
...
```sh
nexuscode bot doctor
nexuscode bot doctor --deep
nexuscode bot start --channels qq,feishu,lark,weixin --dir /path/to/project
```
...
- `credential.app_id`, `credential.app_secret_env`, `credential.account_id`,
  ...
  tokens. Secrets stay in environment variables or the NexusCode user credentials
  store.
...
`nexuscode bot pairing approve <code>` before the sender can drive the bot; when
...
```sh
nexuscode bot pairing list
nexuscode bot pairing approve CODE
nexuscode bot pairing reject CODE
```
...
```sh
export NEXUSCODE_BOT_CONTROL_TOKEN="change-me"

curl -H "Authorization: Bearer $NEXUSCODE_BOT_CONTROL_TOKEN" \
  http://127.0.0.1:37913/status

curl -X POST http://127.0.0.1:37913/send \
  -H "Authorization: Bearer $NEXUSCODE_BOT_CONTROL_TOKEN" \
...
sequenceDiagram
  ...
  participant R as "NexusCode desktop or bot start"
...
| `/projects [query]` | List indexed project workspaces | `/projects nexuscode` |
...
`nexuscode bot doctor --deep` reports queue, pairing, and role diagnostics.
...
- `/search all <query>` searches file contents across indexed project roots.
  NexusCode uses `rg` when available and falls back to a bounded Go scanner.
...
current workspace's `.nexuscode/attachments` directory and passes them to
NexusCode as `@.nexuscode/attachments/...` references. If an attachment cannot be
...
NexusCode bots use the same permission system as the desktop app. Ask mode is the
...
No. A normal NexusCode app upgrade or overwrite install does not require
rebinding.

Bindings are stored in the user's NexusCode data, not inside the app bundle:

- Bot connections, remote IDs, allowlists, model choices, and approval modes are
  stored in the user config.
- Feishu and Lark secrets are stored in NexusCode's global
  `<NexusCode home>/.env`, shared by CLI and desktop.
- The WeChat scanned account token is stored in the NexusCode user data
  directory.
...
- The NexusCode user config directory was deleted.
...
| Connected but no reply | Make sure the desktop bot runtime or `nexuscode bot start` process is running, the bot connection is enabled, and the sender ID is allowlisted, paired, or access is open. |
