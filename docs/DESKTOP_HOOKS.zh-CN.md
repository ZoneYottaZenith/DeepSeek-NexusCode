...
Hooks 让 NexusCode 在会话、用户输入、工具调用、模型返回、压缩上下文等节点执行本地 shell 命令。桌面端在“设置 -> Hooks”里提供图形化编辑入口，本质上读写同一份 `settings.json`。
...
1. 打开桌面端“设置 -> Hooks”。
2. 选择范围：
   - “全局”：保存到 `<NexusCode home>/settings.json`，始终加载；Windows 默认是 `%APPDATA%\nexuscode\settings.json`，macOS/Linux 默认是 `~/.nexuscode/settings.json`。
   - “项目”：保存到当前工作区的 `.nexuscode/settings.json`，必须点击“信任此工作区”后才会加载。
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .nexuscode/hooks/check-bash.js",
    ...
    "Stop": [
      {
        "command": "echo NexusCode turn finished"
      }
    ]
  }
}
...
| 范围 | 文件 | 是否需要信任 | 加载顺序 |
| --- | --- | --- | --- |
| 全局 | `<NexusCode home>/settings.json` | 不需要 | 项目 hooks 之后 |
| 项目 | `<workspace>/.nexuscode/settings.json` | 需要 | 全局 hooks 之前 |

项目 hooks 的信任状态不写在项目文件里，而是写在用户自己的 `<NexusCode home>/trust.json`。这样克隆来的仓库不能靠提交 `.nexuscode/settings.json` 自动执行命令。
...
```json
{
  "hooks": {
    "PreToolUse": [
      { "match": "bash", "command": "node .nexuscode/hooks/pre-tool.js" }
    ],
    "UserPromptSubmit": [
      { "command": "node ~/.nexuscode/hooks/check-prompt.js" }
    ],
    "Stop": [
      { "command": "osascript -e 'display notification \"Turn done\" with title \"NexusCode\"'" }
    ]
  }
}
```
...
```json
[
  { "event": "PreToolUse", "match": "bash", "command": "node .nexuscode/hooks/pre-tool.js" },
...
`command` 会通过平台 shell 执行：macOS/Linux 使用 `sh -c`，Windows 使用 `cmd /c`。stdin 是 NexusCode 写入的一行 JSON，见下面的 payload 表。
...
NexusCode 会把一行 JSON 写入 hook 命令的 stdin。所有 payload 都至少有：
...
- `PostLLMCall`：exit 0 且 stdout 非空时，stdout 会替换用户看到的 reasoning。若 provider 的 reasoning 带签名，NexusCode 会保留原始 signed reasoning 用于后续请求，同时仍展示 hook 转换后的文本。
...
这适合把插件或工作流的 bootstrap 说明带入会话。比如 Superpowers 不需要内置到 NexusCode；可以让它自己的 `hooks/session-start-codex` 在 `SessionStart` 输出 `additionalContext`，或让插件根目录 `CLAUDE.md` 被插件包兼容层直接作为 `SessionStart` 上下文读取，NexusCode 会在下一轮把这段说明注入模型上下文。插件包兼容层也会读取 `.claude/settings.json` 里的 command hooks，并按同名事件映射到 NexusCode hooks。NexusCode 默认允许 `max_subagent_depth = 2`，因此 Superpowers 的父会话或第一层 workflow subagent 可以再派发 reviewer/implementer subagent；第二层不会继续获得递归委派工具。若要恢复旧的单层边界，设 `agent.max_subagent_depth = 1`。这会改变子代理可见工具面，可能影响子代理请求的 prompt cache，但不会把 Superpowers 写进 NexusCode 的稳定 system prompt。

## 示例：阻止危险 bash 命令

`.nexuscode/settings.json`：
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .nexuscode/hooks/block-dangerous-bash.js",
...
`.nexuscode/hooks/block-dangerous-bash.js`：
