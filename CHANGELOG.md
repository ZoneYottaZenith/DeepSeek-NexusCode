...
All notable changes to the Go line (NexusCode 1.0+) are recorded here. The legacy
`0.x` TypeScript history lives on the [`v1`](https://github.com/ZoneYottaZenith/DeepSeek-NexusCode/tree/v1)
branch.
...
- Agent runtime defaults now leave both executor and dedicated planner tool-call
  ...
  come from the user/global config only; project `nexuscode.toml` does not
  override them.
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`ZoneYottaZenith/nexuscode` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `nexuscode codegraph
...
- **Memory**: `NEXUSCODE.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`nexuscode acp`) and an HTTP/SSE server frontend; desktop app (Wails).
...
[1.0.0]: https://github.com/ZoneYottaZenith/DeepSeek-NexusCode/releases/tag/v1.0.0
