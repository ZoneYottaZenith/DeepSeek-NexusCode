...
NexusCode security fixes are prioritized for the currently developed Go rewrite
...
- Affected NexusCode version, commit, operating system, and installation method.
...
NexusCode is a local coding agent. Many features intentionally operate on the
...
NexusCode supports `@path` references so users can include local files and images
...
- A local user intentionally asks NexusCode to read, edit, or send their own
