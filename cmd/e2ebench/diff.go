...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "nexuscode.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 NexusCode e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "nexuscode/internal/agent/foo.go" into
