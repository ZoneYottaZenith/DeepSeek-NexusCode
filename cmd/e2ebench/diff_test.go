...
func TestEnv(t *testing.T) {
	if got := os.Getenv("NEXUSCODE_E2E_ENV"); got != "ok" {
		t.Fatalf("NEXUSCODE_E2E_ENV = %q", got)
	}
}
`)

	ok, out := runTests(repo, "GOWORK=off NEXUSCODE_E2E_ENV=ok go test", []string{"./..."})
	if !ok {
...
func TestRunTestsRejectsDynamicEnvAssignment(t *testing.T) {
	ok, out := runTests(t.TempDir(), "NEXUSCODE_E2E_ENV=$(echo ok) go test", []string{"./..."})
	if ok {
