...
type Agent struct {
	...
	// may write a NexusCode-managed config file outside the workspace roots.
...
// the file tools when a target is a NexusCode-managed config file outside the
...
// `nexuscode --resume` to load a saved JSONL transcript before the first turn,
...
type Options struct {
	...
	// ConfigWriteApprover confirms file-tool writes to NexusCode-managed config
