...
func TestHandoffTaskRecoversOriginalInput(t *testing.T) {
	...
	for _, plain := range []string{"ordinary input", "", "# NexusCode executor handoff with no sections"} {
...
func TestCoordinatorPlannerUsesReadOnlyResearchTools(t *testing.T) {
	planner := &mockProvider{name: "planner", streams: [][]provider.Chunk{
		{
			{Type: provider.ChunkToolCall, ToolCall: &provider.ToolCall{ID: "call-1", Name: "read_file", Arguments: `{"path":"NEXUSCODE.md"}`}},
			{Type: provider.ChunkDone},
		},
		{
...
func TestCoordinatorHandoffAffirmsExecutorToolSchemasWhenPlannerClaimsNoMCP(t *testing.T) {
	...
	exec := &mockProvider{name: "executor", streams: [][]provider.Chunk{
		...
		{
			{Type: provider.ChunkToolCall, ToolCall: &provider.ToolCall{ID: "call-1", Name: "mcp__github__search", Arguments: `{"query":"NexusCode discussions"}`}},
			{Type: provider.ChunkDone},
		},
		{
			{Type: provider.ChunkText, Text: "Done."},
