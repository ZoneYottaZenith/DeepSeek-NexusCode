...
// directory. It is gated behind NEXUSCODE_BENCH=1 so it never runs (or writes
// hundreds of MB) during a normal `go test`.
//
//	NEXUSCODE_BENCH=1 go test ./internal/agent -run TestListSessionsScalingManual -v
func TestListSessionsScalingManual(t *testing.T) {
	if os.Getenv("NEXUSCODE_BENCH") != "1" {
		t.Skip("set NEXUSCODE_BENCH=1 to run the scaling benchmark")
	}
...
//
//	NEXUSCODE_BENCH=1 go test ./internal/agent -run TestColdStartProjectTreeManual -v
func TestColdStartProjectTreeManual(t *testing.T) {
	if os.Getenv("NEXUSCODE_BENCH") != "1" {
		t.Skip("set NEXUSCODE_BENCH=1 to run the cold-start benchmark")
	}
