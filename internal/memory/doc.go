// Package memory implements NexusCode's persistent memory. It mirrors Claude
// Code's two-layer model while honoring NexusCode's cache-first architecture:
//
//   - Hierarchical doc memory: NEXUSCODE.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.nexuscode/NEXUSCODE.md
	ScopeAncestor Scope = "ancestor" // a NEXUSCODE.md above the project root
	ScopeProject  Scope = "project"  // ./NEXUSCODE.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./NEXUSCODE.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// NEXUSCODE.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"NEXUSCODE.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"NEXUSCODE.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
