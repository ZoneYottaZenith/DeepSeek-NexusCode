...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the NexusCode home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.nexuscode/skills/ (this workspace only); 'global' writes to the NexusCode home skills directory (every project)."
	}
