...
func TestRunSkillInline(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/note.md", "---\ndescription: take a note\n---\nDo the thing.")
...
func TestRunSkillSubagentNeedsRunner(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/dig.md", "---\ndescription: dig\nrunAs: subagent\n---\nbody")
...
func TestReadOnlySkillSubagentResolvesProfile(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/deep.md", "---\ndescription: deep\nrunAs: subagent\nmodel: deepseek-pro\neffort: max\n---\nbody")
	tl := NewReadOnlySkillTool(New(Options{HomeDir: home, DisableBuiltins: true}), nil)
...
func TestInstallSkill(t *testing.T) {
	...
	wantPath := filepath.Join(home, ".nexuscode", "skills", "deploy", SkillFile)
	...
	if _, err := os.Stat(filepath.Join(home, ".nexuscode", "skills", "deploy.md")); !errors.Is(err, os.ErrNotExist) {
