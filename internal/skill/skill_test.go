...
func TestListPrecedenceProjectOverGlobal(t *testing.T) {
	home := t.TempDir()
	proj := t.TempDir()
	writeSkill(t, proj, ".nexuscode/skills/greet.md", "---\nname: greet\ndescription: project greet\n---\nproject body")
	writeSkill(t, home, ".nexuscode/skills/greet.md", "---\ndescription: global greet\n---\nglobal body")
	writeSkill(t, home, ".nexuscode/skills/onlyglobal.md", "---\ndescription: only global\n---\nbody")
...
func TestFlatAndDirLayout(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/flat.md", "---\ndescription: flat\n---\nflat body")
	writeSkill(t, home, ".nexuscode/skills/dir/SKILL.md", "---\ndescription: dir\n---\ndir body")
...
func TestNestedSkillsDiscoveredByDefault(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/superpower/skill-a.md", "---\ndescription: nested flat\n---\nflat body")
	writeSkill(t, home, ".nexuscode/skills/superpower/tool-a/SKILL.md", "---\ndescription: nested dir\n---\ndir body")
	writeSkill(t, home, ".nexuscode/skills/superpower/references/notes.md", "---\ndescription: not a skill\n---\nnotes")
...
func TestNestedSkillsRequireDescription(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/root.md", "---\n---\nroot body")
	writeSkill(t, home, ".nexuscode/skills/superpower/draft.md", "---\n---\ndraft body")
	writeSkill(t, home, ".nexuscode/skills/superpower/tool/SKILL.md", "---\n---\ntool body")
...
func TestNestedDirectorySkillStopsTraversal(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/pack/SKILL.md", "---\ndescription: pack\n---\npack body")
	writeSkill(t, home, ".nexuscode/skills/pack/child.md", "---\ndescription: child\n---\nchild body")
...
func TestNexusCodeHomeDirOverridesGlobalNexusCodeSkills(t *testing.T) {
	home := t.TempDir()
	nexuscodeHome := filepath.Join(t.TempDir(), "rx-home")
	writeSkill(t, home, ".nexuscode/skills/old.md", "---\ndescription: old\n---\nold")
	writeSkill(t, home, ".nexuscode/skills/current.md", "---\ndescription: old current\n---\nold current")
	currentPath := writeSkill(t, nexuscodeHome, "skills/current.md", "---\ndescription: current\n---\ncurrent")

	st := New(Options{HomeDir: home, NexusCodeHomeDir: nexuscodeHome, DisableBuiltins: true})
	list := st.List()
	current, ok := find(list, "current")
	if !ok {
		t.Fatal("NexusCode home skill should be discovered")
	}
	if current.Path != currentPath {
		t.Fatalf("current skill path = %q, want NexusCode home path %q", current.Path, currentPath)
	}
	if _, ok := find(list, "old"); !ok {
		t.Fatal("legacy ~/.nexuscode skill should remain discoverable")
	}
	...
	want := filepath.Join(nexuscodeHome, SkillsDirname, "created", SkillFile)
	if path != want {
...
func TestExcludedPathsHideConventionRoots(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/keep.md", "---\ndescription: keep\n---\nb")
...
func TestFrontmatterFields(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/sub.md",
		"---\ndescription: a sub\nrunAs: subagent\nallowed-tools: read_file, grep\nmodel: deepseek-pro\nread-only: true\n---\nbody")
	writeSkill(t, home, ".nexuscode/skills/fork.md", "---\ndescription: f\ncontext: fork\n---\nbody")
	writeSkill(t, home, ".nexuscode/skills/plain.md", "---\ndescription: p\n---\nbody")
...
func TestReferencesInlined(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/withrefs/SKILL.md", "---\ndescription: r\n---\nmain body")
	writeSkill(t, home, ".nexuscode/skills/withrefs/references/b.md", "second ref")
	writeSkill(t, home, ".nexuscode/skills/withrefs/references/a.md", "first ref")
...
func TestScriptsAppended(t *testing.T) {
	...
	writeSkill(t, home, ".nexuscode/skills/withscripts/SKILL.md", "---\ndescription: r\n---\nmain body")
	writeScript(t, home, ".nexuscode/skills/withscripts/scripts/lint.py", "#!/usr/bin/env python3\nprint('ok')")
	writeScript(t, home, ".nexuscode/skills/withscripts/scripts/deploy.sh", "#!/usr/bin/env bash\necho ok")
...
func TestNoScriptsWhenDirAbsent(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/noscripts/SKILL.md", "---\ndescription: r\n---\nmain body")
	st := New(Options{HomeDir: home, DisableBuiltins: true})
...
func TestScriptsFilteredByExt(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/scriptscheck/SKILL.md", "---\ndescription: t\n---\nbody")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/lint.py", "#!/usr/bin/env python3\nprint('ok')\n")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/.hidden.py", "")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/readme.md", "# readme")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/deploy", "#!/bin/sh\necho ok")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/legacy.p", "print 'ok'\n")
	writeScript(t, home, ".nexuscode/skills/scriptscheck/scripts/.gitkeep", "")
...
func TestBuiltinsPresentAndOverridable(t *testing.T) {
	...
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/explore.md", "---\ndescription: mine\nrunAs: inline\n---\nbody")
...
func TestDisabledSkillsAreFilteredFromListAndRead(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/active.md", "---\ndescription: active\n---\nbody")
	writeSkill(t, home, ".nexuscode/skills/hidden.md", "---\ndescription: hidden\n---\nbody")
...
func TestInvalidNamesSkipped(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".nexuscode/skills/bad name.md", "---\ndescription: x\n---\nb") // space → invalid
	st := New(Options{HomeDir: home, DisableBuiltins: true})
	if len(st.List()) != 0 {
...
func TestSymlinkedDirAndFile(t *testing.T) {
	...
	skillsRoot := filepath.Join(home, ".nexuscode", "skills")
...
func TestCreateRefusesOverwrite(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".nexuscode", "skills", "mine", SkillFile)) {
	...
	writeSkill(t, home, ".nexuscode/skills/legacy.md", "---\ndescription: legacy\n---\nbody")
