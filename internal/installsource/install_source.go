...
func NewTool(opts Options) tool.Tool {
	...
	} else if dir := config.NexusCodeHomeDir(); dir != "" {
		nexuscodeHome = dir
	} else if home != "" {
		nexuscodeHome = filepath.Join(home, ".nexuscode")
	}
...
func (t *installSourceTool) Execute(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if len(actions) == 0 {
		out := response{
			...
			PlanID:   planID,
			Warnings: warnings,
			Next:     "No installable NexusCode skill, MCP server, or plugin package was detected. Ask the user for a direct SKILL.md, skill root, .mcp.json, plugin manifest, MCP endpoint, or package name.",
		}
...
func (t *installSourceTool) uninstallActionsForScope(name, scope string) []action {
	...
	if scope == "global" || scope == "" {
		if st, err := pluginpkg.LoadState(t.nexuscodeHome); err == nil {
			for _, p := range st.Plugins {
				if p.Name != name {
					continue
				}
				root := pluginpkg.ResolveRoot(t.nexuscodeHome, p.Root)
				actions = append(actions, action{
					Kind:         "plugin",
					...
					Target:       root,
					Scope:        "global",
					ConfigPath:   pluginpkg.StatePath(t.nexuscodeHome),
...
func (t *installSourceTool) resolveSkillPath(name, scope string) (string, bool) {
	...
	if scope == "global" {
		...
		root = filepath.Join(t.nexuscodeHome, skill.SkillsDirname)
	} else {
		root = filepath.Join(t.root, ".nexuscode", skill.SkillsDirname)
	}
...
func (t *installSourceTool) configPath(scope string) string {
	...
	return filepath.Join(t.root, "nexuscode.toml")
}
