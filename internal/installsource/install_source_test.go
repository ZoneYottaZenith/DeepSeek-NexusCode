...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "nexuscode-installsource-test-*")
	if err != nil {
...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "nexuscode.toml"))
...
func TestApplyLocalCodexPluginPackage(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	statePath := filepath.Join(home, ".nexuscode", "plugin-packages.json")
	...
	if _, err := os.Stat(filepath.Join(home, ".nexuscode", "plugins", "superpowers", ".codex-plugin", "plugin.json")); err != nil {
...
func TestApplyLocalClaudePluginPackage(t *testing.T) {
  ...
  "skills": "./.claude/skills/"
	...
	if _, err := os.Stat(filepath.Join(home, ".nexuscode", "plugins", "ui-ux-pro-max", ".claude-plugin", "plugin.json")); err != nil {
		t.Fatalf("installed plugin missing: %v", err)
	}
}
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".nexuscode", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".nexuscode", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".nexuscode", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".nexuscode", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".nexuscode", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".nexuscode", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "nexuscode.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "nexuscode.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "nexuscode.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "nexuscode.toml"))
...
func TestApplyMCPRollsBackOnSaveFailure(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(project, "nexuscode.toml"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(project, "nexuscode.toml", "blocker"), "x")
...
func TestGlobalSkillInstallRootUsesNexusCodeHome(t *testing.T) {
	home := t.TempDir()
	...
	want := filepath.Join(nexuscodeHome, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".nexuscode", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".nexuscode", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".nexuscode", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/nexuscode.toml"}
	b := a
