...
func (t *installSourceTool) planGitHubPluginPackage(ctx context.Context, req request) ([]action, []string, error) {
	...
	for _, branch := range src.branches() {
		for _, manifestPath := range pluginpkg.ManifestPaths() {
			...
			tmp, err := os.MkdirTemp("", "nexuscode-plugin-plan-*")
			if err != nil {
...
func (t *installSourceTool) pluginPackageAction(req request, pkg pluginpkg.Package, source string) action {
	...
	root := ""
	if t.nexuscodeHome != "" {
		root = pluginpkg.InstallRoot(t.nexuscodeHome, name)
	}
	...
	if hooks > 0 {
		a.RiskReasons = append(a.RiskReasons, "registers shell hooks that execute during NexusCode sessions")
	}
	if mcp > 0 {
...
func (t *installSourceTool) applyInstallPluginPackage(ctx context.Context, req request, act *action) error {
	if t.nexuscodeHome == "" {
		return newErr(ErrSourceUnreadable, "plugin install requires a NexusCode home directory")
	}
	if !pluginpkg.IsValidName(act.Name) {
		return newErr(ErrInvalidManifest, "invalid plugin name %q", act.Name)
	}
	target := pluginpkg.InstallRoot(t.nexuscodeHome, act.Name)
	...
	installed := pluginpkg.InstalledPlugin{
		Name:         act.Name,
		Source:       act.Source,
		Root:         pluginpkg.RelativeRoot(t.nexuscodeHome, target),
		Version:      pkg.Manifest.Version,
	...
	if err := pluginpkg.Upsert(t.nexuscodeHome, installed); err != nil {
		return err
	}
	act.Target = target
...
func (t *installSourceTool) preparePluginSource(ctx context.Context, source, mode string) (string, func(), error) {
	...
	if isURL(source) {
		...
		tmp, err := os.MkdirTemp("", "nexuscode-plugin-*")
		if err != nil {
			return "", func() {}, err
		}
...
func (t *installSourceTool) applyRemovePluginPackage(_ request, act *action) error {
	installed, ok, err := pluginpkg.Remove(t.nexuscodeHome, act.Name)
	if err != nil || !ok {
		return err
	}
	root := pluginpkg.ResolveRoot(t.nexuscodeHome, installed.Root)
	...
	pluginsDir := pluginpkg.PluginsDir(t.nexuscodeHome)
