...
func TestResearchGoalCreatesHostManagedAutoResearchTask(t *testing.T) {
	...
	for _, rel := range []string{
		...
		path := filepath.Join(root, ".nexuscode", "autoresearch", state.AutoResearchTaskID, rel)
	...
	readJSONFileForTest(t, filepath.Join(root, ".nexuscode", "autoresearch", state.AutoResearchTaskID, "state", "task_spec.json"), &spec)
...
func TestResearchGoalRepeatedSetReusesAutoResearchTask(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, ".nexuscode", "autoresearch"))
	if err != nil {
...
func TestResearchGoalMissingExplicitTaskBlocksInsteadOfCreatingNewTask(t *testing.T) {
	...
	c.SetGoalWithResearchMode("resume .nexuscode/autoresearch/missing-task/", GoalResearchOn)
