...
const (
	...
	refImage                   // a local image attachment: @.nexuscode/attachments/<file>
)
...
const externalFolderRefPrefix = "__nexuscode_external_folder"
...
func isAttachmentRef(token string) bool {
	return strings.HasPrefix(filepath.ToSlash(token), ".nexuscode/attachments/")
}
...
// text rather than a NexusCode slash command.
