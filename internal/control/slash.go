...
func (c *Controller) managementNotice(trimmed string) bool {
	...
	case "/plugin", "/plugins":
		...
		switch sub {
		case "", "list", "ls":
			text, err := pluginpkg.InstalledListText(config.NexusCodeHomeDir())
			if err != nil {
		...
		case "show", "cat":
			...
			text, err := pluginpkg.InstalledShowText(config.NexusCodeHomeDir(), fields[2])
			if err != nil {
				c.notice("plugins: " + err.Error())
			} else {
				c.notice(text)
	...
	case "/hooks":
		...
		case "trust":
			...
			if err := hook.Trust(root, ""); err != nil {
				...
				c.notice("trusted this project's hooks — restart NexusCode to load them")
			}
		default:
