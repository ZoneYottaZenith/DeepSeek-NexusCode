...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".nexuscode/attachments/clipboard-20260601-010203.000000.png": true,
		".nexuscode/attachments/clipboard-20260601-010203.000000.yml": true,
		".nexuscode/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".nexuscode/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".nexuscode/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".nexuscode/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".nexuscode", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".nexuscode/attachments/config.yml")
	zipRef := filepath.ToSlash(".nexuscode/attachments/archive.zip")
	pngRef := filepath.ToSlash(".nexuscode/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: nexuscode\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: nexuscode") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".nexuscode", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".nexuscode/attachments/shot.png" {
