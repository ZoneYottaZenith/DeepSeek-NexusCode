...
func TestSaveImageDataURL(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".nexuscode/attachments/clipboard-") || !strings.HasSuffix(got, ".png") {
...
func TestSaveAttachmentFile(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".nexuscode/attachments/clipboard-") || !strings.HasSuffix(got, ".pdf") {
...
func TestImageDataURLRejectsOutsideAttachmentDir(t *testing.T) {
	...
	if _, err := ImageDataURL("../.nexuscode/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkFile(t *testing.T) {
	...
	link := filepath.Join(".nexuscode", "attachments", "link.png")
...
func TestImageDataURLRejectsSymlinkAttachmentDir(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.Mkdir(".nexuscode", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("elsewhere", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("../elsewhere", filepath.Join(".nexuscode", "attachments")); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if _, err := ImageDataURL(".nexuscode/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkSubdirectory(t *testing.T) {
	...
	link := filepath.Join(".nexuscode", "attachments", "link")
