...
import (
	...
	"nexuscode/internal/autoresearch"
	"nexuscode/internal/billing"
	"nexuscode/internal/checkpoint"
	"nexuscode/internal/command"
	"nexuscode/internal/config"
	"nexuscode/internal/diff"
	"nexuscode/internal/event"
	"nexuscode/internal/evidence"
...
type Controller struct {
	...
	// nexuscode.toml on add/remove, building specs from entries). See mcp.go.
	mcp mcpManager
...
// approval when a file tool targets a NexusCode-managed config file outside the
...
// headless `nexuscode run` path, where the Sink renders to stdout and the caller
...
// out of project nexuscode.toml while preserving the CLI AddMCPServer semantics.
...
// NEXUSCODE.md by default) — the write side of "#<note>". Returns the file written.
...
// managedConfigWriteApprover routes a file tool's NexusCode-managed config write
...
func listSourceDirs(root string, maxDepth int) []string {
	skip := map[string]bool{
		...
		"vendor": true, ".nexuscode": true, "desktop": true,
