...
- Say briefly in the first visible reply that the goal is being handled with AutoResearch and that host-owned state lives under .nexuscode/autoresearch/<task-id>/, using the actual task_id from <autoresearch-runtime>.
- Keep dynamic state out of NEXUSCODE.md, AGENTS.md, project memory, system prompts, and tool schemas. Use project-local .nexuscode/autoresearch/ state only.
...
- Complete only after auditing every open success criterion in <autoresearch-runtime> against direct evidence. Public publishing, destructive changes, credential use, payments, external notifications, privacy-sensitive output, and cache-sensitive changes still require the normal NexusCode gates.`
...
func shouldAutoStartResearchGoal(input string) bool {
	...
	if strings.Contains(lower, ".nexuscode/autoresearch/") {
		return true
	}
