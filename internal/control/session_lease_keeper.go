...
// that binds session files for writing (the CLI chat/run commands, `nexuscode
...
const SessionLeaseCloseHint = "close the other NexusCode window or process first"
...
func SessionInUseMessage(err error) string {
	const fallback = "this session is in use by another NexusCode window or process"
	...
	fmt.Fprintf(&b, "this session is in use by another NexusCode process (pid %d", info.PID)
