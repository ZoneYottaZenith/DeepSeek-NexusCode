...
func TestGatewaySessionsSearchAndAttachSessionOverride(t *testing.T) {
	t.Setenv("NEXUSCODE_HOME", t.TempDir())
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
...
func TestGatewayDefaultQueueSteersMediaOnlyActiveTurn(t *testing.T) {
	...
	got := ctrl.steered()
	if len(got) != 1 || !strings.Contains(got[0], "Attachments:") || !strings.Contains(got[0], "@.nexuscode/attachments/") {
...
func TestGatewayUnknownDMGetsPairingCode(t *testing.T) {
	...
	sent := adapter.sentMessages()
	if len(sent) != 1 || !strings.Contains(sent[0].Text, "配对码") || !strings.Contains(sent[0].Text, "nexuscode bot pairing approve") {
...
func TestGatewayControlServerStatusAndSend(t *testing.T) {
	...
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusOK || !strings.Contains(string(metricsBody), "nexuscode_bot_adapter_sends_total") {
