...
type GatewayConfig struct {
	...
	// session. Nil when the gateway runs standalone (nexuscode bot start).
...
// BotGateway 是 nexuscode bot 消息网关，管理 Controller 生命周期、session 并发、
// 事件渲染和平台适配器。
...
func (gw *BotGateway) offerPairing(ctx context.Context, adapter Adapter, msg InboundMessage) bool {
	...
	text := fmt.Sprintf("%s\n配对码: %s\n请在本机运行: nexuscode bot pairing approve %s\n此码将在 %s 过期。",
