...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/nexuscode-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/nexuscode-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/nexuscode-sed-test/e' file.txt", false},
