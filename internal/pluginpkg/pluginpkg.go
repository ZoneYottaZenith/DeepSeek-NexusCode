// Package pluginpkg handles installed NexusCode plugin packages.
//
...
import (
	...
	"nexuscode/internal/fileutil"
	fileencoding "nexuscode/internal/fileutil/encoding"
	"nexuscode/internal/frontmatter"
)

const (
	NativeManifest = "nexuscode-plugin.json"
...
// Manifest is the normalized manifest shape used by NexusCode.
...
// State is persisted at <NexusCode home>/plugin-packages.json.
type State struct {
...
func StatePath(nexuscodeHome string) string {
	return filepath.Join(nexuscodeHome, StateFilename)
}

func PluginsDir(nexuscodeHome string) string {
	return filepath.Join(nexuscodeHome, "plugins")
}

func InstallRoot(nexuscodeHome, name string) string {
	return filepath.Join(PluginsDir(nexuscodeHome), name)
}

func LoadState(nexuscodeHome string) (State, error) {
	var st State
	b, err := fileencoding.ReadFileUTF8(StatePath(nexuscodeHome))
	if err != nil {
...
func SaveState(nexuscodeHome string, st State) error {
	if st.Version == 0 {
		st.Version = 1
	}
	sort.SliceStable(st.Plugins, func(i, j int) bool { return st.Plugins[i].Name < st.Plugins[j].Name })
	...
	b = append(b, '\n')
	return fileutil.AtomicWriteFile(StatePath(nexuscodeHome), b, 0o644)
}
...
// not a cross-process lock — concurrent NexusCode processes can still race.
var stateMu sync.Mutex

func Upsert(nexuscodeHome string, p InstalledPlugin) error {
	if !IsValidName(p.Name) {
		return fmt.Errorf("invalid plugin name %q", p.Name)
	}
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(nexuscodeHome)
	if err != nil {
		return err
	}
	for i := range st.Plugins {
		if st.Plugins[i].Name == p.Name {
			st.Plugins[i] = p
			return SaveState(nexuscodeHome, st)
		}
	}
...
func Remove(nexuscodeHome, name string) (InstalledPlugin, bool, error) {
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(nexuscodeHome)
	if err != nil {
	...
	for i, p := range st.Plugins {
		...
		return p, true, SaveState(nexuscodeHome, st)
	}
	return InstalledPlugin{}, false, nil
}

func SetEnabled(nexuscodeHome, name string, enabled bool) error {
	stateMu.Lock()
	defer stateMu.Unlock()
	st, err := LoadState(nexuscodeHome)
	if err != nil {
...
func LoadInstalled(nexuscodeHome string) ([]InstalledPackage, []string) {
	st, err := LoadState(nexuscodeHome)
	if err != nil {
	...
	for _, installed := range st.Plugins {
		...
		root := ResolveRoot(nexuscodeHome, installed.Root)
...
func ResolveRoot(nexuscodeHome, root string) string {
	if filepath.IsAbs(root) {
		return filepath.Clean(root)
	}
	return filepath.Join(nexuscodeHome, filepath.Clean(root))
}

func RelativeRoot(nexuscodeHome, root string) string {
	if rel, err := filepath.Rel(nexuscodeHome, root); err == nil && rel != "." && !strings.HasPrefix(rel, ".."+string(filepath.Separator)) && rel != ".." {
...
func parseNative(path, root string) (Package, []string, error) {
	...
	return Package{Root: root, ManifestKind: "nexuscode", Manifest: manifest}, warnings, nil
}
...
// NexusCode does not map yet. Their presence is worth a warning: silently
...
// shape), and reports the conventional capabilities NexusCode cannot map.
func applyClaudeConventionDirs(root string, manifest *Manifest) []string {
	...
	for _, rel := range claudeUnmappedCapabilities {
		if _, err := os.Stat(filepath.Join(root, filepath.FromSlash(rel))); err == nil {
			warnings = append(warnings, fmt.Sprintf("claude plugin declares %s, which NexusCode does not map yet; that capability will not be installed", rel))
		}
	}
	return warnings
}
