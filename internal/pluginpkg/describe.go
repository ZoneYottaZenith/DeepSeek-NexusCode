...
func InstalledNames(nexuscodeHome string) ([]string, error) {
...
func InstalledListText(nexuscodeHome string) (string, error) {
	st, err := LoadState(nexuscodeHome)
	if err != nil {
		return "", err
	}
	if len(st.Plugins) == 0 {
		return "plugins: none installed\ninstall: nexuscode plugin install <source> --yes, or use Settings -> Plugins", nil
	}
	...
	for _, p := range st.Plugins {
		...
		counts := pluginCapabilityText(nexuscodeHome, p)
		if summary != "" && counts != "" {
...
func InstalledShowText(nexuscodeHome, name string) (string, error) {
	p, ok, err := FindInstalled(nexuscodeHome, name)
	if err != nil {
		return "", err
	}
	if !ok {
	...
	root := ResolveRoot(nexuscodeHome, p.Root)
...
func FindInstalled(nexuscodeHome, name string) (InstalledPlugin, bool, error) {
	st, err := LoadState(nexuscodeHome)
	if err != nil {
...
func pluginCapabilityText(nexuscodeHome string, p InstalledPlugin) string {
	root := ResolveRoot(nexuscodeHome, p.Root)
