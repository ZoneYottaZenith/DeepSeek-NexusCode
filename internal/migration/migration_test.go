...
func migrationRescueHome(t *testing.T) string {
	...
	t.Setenv("NEXUSCODE_STATE_HOME", filepath.Join(home, "new-state"))
...
func isolateMigrationHome(t *testing.T) string {
	...
	t.Setenv("NEXUSCODE_HOME", filepath.Join(home, "new-nexuscode"))
...
func TestRunLegacyRescueImportsSessionsAndEmitsProgress(t *testing.T) {
	home := migrationRescueHome(t)
	legacyDir := filepath.Join(home, ".nexuscode", "sessions")
...
func TestRunLegacyRescueImportsMemory(t *testing.T) {
	home := migrationRescueHome(t)
	legacyRoot := filepath.Join(home, ".nexuscode")
	if err := os.MkdirAll(filepath.Join(legacyRoot, "memory", "global"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacyRoot, "NEXUSCODE.md"), []byte("legacy user memory\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		filepath.Join(config.MemoryUserDir(), "NEXUSCODE.md"),
...
func TestRunLegacyRescueSkipsImplicitSourcesWhenIsolated(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(config.MemoryUserDir(), "NEXUSCODE.md")); !os.IsNotExist(err) {
		t.Fatalf("isolated rescue imported legacy memory, stat err=%v", err)
	}
	joined := strings.Join(notices, "\n")
	if !strings.Contains(joined, "NEXUSCODE_HOME is set; implicit legacy migration is skipped") {
...
func TestRunLegacyRescueCommandImportsFromExplicitInstallDir(t *testing.T) {
	home := isolateMigrationHome(t)
	installRoot := filepath.Join(home, "Custom NexusCode")
...
func TestMigrateLegacySessionSourcesSkipsCurrentProjectTree(t *testing.T) {
	...
	t.Setenv("NEXUSCODE_HOME", "")
	t.Setenv("NEXUSCODE_STATE_HOME", "")
	if !samePath(config.MemoryUserDir(), filepath.Join(home, ".nexuscode")) {
		t.Skip("current NexusCode home is not ~/.nexuscode on this platform")
	}
