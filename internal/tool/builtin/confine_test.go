...
func TestManagedConfigWriteFailsClosedWithoutApprover(t *testing.T) {
	...
	cfg := config.Default()
	managed := NewManagedConfigPaths(config.NexusCodeManagedConfigPaths())
...
func TestManagedConfigWriteGatedOnApprover(t *testing.T) {
	...
	decline := &stubConfigWriteApprover{allow: false, reason: "the user declined this NexusCode config write"}
	...
	// Even with an always-allowing approver, non-config files in the NexusCode
	...
	for _, target := range []string{
		...
		filepath.Join(home, ".nexuscode", ".env"),
		filepath.Join(home, ".nexuscode", "settings.json"),
		filepath.Join(home, ".nexuscode", "skills", "evil", "SKILL.md"),
	} {
...
func TestBashSandboxConfinement(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".nexuscode-bashsb-*")
	if err != nil {
	...
	outPath := filepath.Join(home, ".nexuscode-bashsb-escape.txt")
