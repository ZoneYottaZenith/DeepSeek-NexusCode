...
// warning to command output when the command references NexusCode's own session
...
// ConfineWebFetch returns the web_fetch built-in bound to NexusCode proxy
...
// guard additionally rejects writes into NexusCode's own session stores even
// when the roots would allow them (see SessionDataGuard). managed names the
// NexusCode-owned config files writable outside the roots after a fresh human
...
// user enabled [secrets] protect_sensitive_files, matches NexusCode's built-in
...
func confine(roots []string, target string) error {
	...
	return fmt.Errorf("path %q is outside the writable roots (writes are confined to %s); "+
		"write inside the workspace or a configured allow_write root, or widen [sandbox] workspace_root / allow_write in nexuscode.toml",
...
// it targets NexusCode's own session stores. A target outside every root that
// matches a NexusCode-managed config file (see ManagedConfigPaths) may proceed
