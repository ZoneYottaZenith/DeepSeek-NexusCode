...
// stateRootFor builds a fake NexusCode state root with the two guarded session
...
func TestSessionDataGuardDeniesSessionStores(t *testing.T) {
	...
	for _, target := range []string{
		...
		} else if !strings.Contains(err.Error(), "NexusCode's own session/state data") {
...
func TestSessionDataGuardZeroValueUnconfined(t *testing.T) {
	...
	if hint := g.CommandHint("", "rm -rf ~/.nexuscode/sessions"); hint != "" {
...
func TestSessionDataGuardCommandHintEnvVarForm(t *testing.T) {
	...
	state := filepath.Join(home, ".nexuscode")
	...
	for _, cmd := range []string{
		`python3 -c "open('$HOME/.nexuscode/sessions/x.jsonl','w')"`,
		"rm ${HOME}/.nexuscode/projects/slug/sessions/y.jsonl",
	} {
		if hint := g.CommandHint("", cmd); hint == "" {
...
func TestBashAppendsSessionDataHint(t *testing.T) {
	...
	if !strings.Contains(out, "WARNING: this command referenced NexusCode's own session/state data") {
