...
func TestBashMergesLoginShellPath(t *testing.T) {
	...
	probe := filepath.Join(bin, "nexuscode-path-probe")
	...
	args, _ := json.Marshal(map[string]string{"command": "nexuscode-path-probe"})
...
func TestBashCommandEnvFiltersSensitiveKeysWhenEnabled(t *testing.T) {
	...
	t.Setenv("NEXUSCODE_TEST_VISIBLE", "ok")
	...
	if !strings.Contains(env, "NEXUSCODE_TEST_VISIBLE=ok") {
