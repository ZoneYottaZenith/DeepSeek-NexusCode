...
// to drive NexusCode. Many tools integrated with the v1 (main-branch) agent over
...
// older map shape that NexusCode v1 clients used.
type MCPEnv map[string]string

// MCPHeaders accepts ACP's official HTTPHeader[] shape while still accepting
// the older map shape that NexusCode v1 clients used. The official spec
...
// SessionListResult is the first and only page of sessions NexusCode currently
...
// unsaved editor state. Line (1-based) and Limit page the content; NexusCode
