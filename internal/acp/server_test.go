...
func startServer(t *testing.T, factory Factory) (*rpcClient, func()) {
	...
	go func() {
		_ = Serve(context.Background(), inR, outW, factory, AgentInfo{Name: "nexuscode-test", Version: "0"})
		close(done)
	}()
...
func TestServeLifecycle(t *testing.T) {
	...
	if len(ir.AuthMethods) != 1 || ir.AuthMethods[0].ID != "nexuscode-setup" || ir.AuthMethods[0].Type != "terminal" {
		t.Fatalf("authMethods = %+v, want terminal nexuscode setup", ir.AuthMethods)
	}
	if len(ir.AuthMethods[0].Args) != 1 || ir.AuthMethods[0].Args[0] != "setup" {
		t.Fatalf("auth args = %+v, want [setup]", ir.AuthMethods[0].Args)
	}

	authResp := client.call(t, "authenticate", AuthenticateParams{MethodID: "nexuscode-setup"})
