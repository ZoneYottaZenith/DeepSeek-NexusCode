...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("NEXUSCODE_TEST_SECRET", "sk-live-secret")
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:     "model-a",
		APIKeyEnv: "NEXUSCODE_TEST_SECRET",
	}}
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	t.Setenv("NEXUSCODE_HOME", filepath.Join(t.TempDir(), "nexuscode"))
	...
	if !strings.Contains(text, "nexuscode 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config nexuscode.toml: parse boom"}})
