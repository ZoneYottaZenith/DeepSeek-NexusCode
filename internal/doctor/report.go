...
func Collect(opts Options) Report {
	...
	// Settings UIs and `nexuscode config` edit the user-level config, but a
	// project nexuscode.toml outranks it. Users who toggle the sandbox off in
	// Settings while the project file pins [sandbox] read the no-op as "bash is
	// broken" (#5961, #6046) — surface the layering explicitly.
	if sourcePath != "" && filepath.Base(sourcePath) == "nexuscode.toml" {
...
func RenderText(r Report) string {
	...
	fmt.Fprintf(&b, "nexuscode %s doctor\n", r.Version)
