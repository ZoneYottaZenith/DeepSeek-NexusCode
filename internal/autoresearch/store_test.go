...
func TestCreateTaskCreatesHostOwnedLayoutAndInitialState(t *testing.T) {
	...
	wantRoot := filepath.Join(root, ".nexuscode", "autoresearch", task.ID)
...
func TestLoadTaskRejectsSymlinkTaskDirectory(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".nexuscode", "autoresearch"), 0o755); err != nil {
		t.Fatalf("create autoresearch root: %v", err)
	}
	if err := os.Symlink(outside, filepath.Join(root, ".nexuscode", "autoresearch", taskID)); err != nil {
...
func TestResumeFromGoalTextLoadsExplicitTaskPath(t *testing.T) {
	...
	resumed, ok, err := store.ResumeFromGoalText("继续 .nexuscode/autoresearch/" + task.ID + "/ 这个任务")
	if err != nil {
