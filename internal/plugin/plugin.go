// Package plugin is NexusCode's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision NexusCode advertises during initialize.
...
type Spec struct {
	...
	// NexusCode's built-in defaultCallTimeout.
	...
	// inherit nexuscode's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — NexusCode is a
...
func (c *Client) initializeSession(ctx context.Context, recordCapabilities bool) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "nexuscode", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into NexusCode's parallel-dispatch
