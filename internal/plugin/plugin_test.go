...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "nexuscode-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "nexuscode-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "nexuscode-missing-mcp-binary" not found on PATH`,
