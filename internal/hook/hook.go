...
// (.nexuscode/settings.json, only when the project is trusted) and a global
// (<NexusCode home>/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".nexuscode"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is <NexusCode home>/settings.json (homeDir overrides ~ for
// tests and legacy callers).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(nexuscodeHome(homeDir), SettingsFilename)
}

// ProjectSettingsPath is <root>/.nexuscode/settings.json.
...
func Load(opts LoadOptions) []ResolvedHook {
	...
	appendPluginHooks(&out, nexuscodeHome(opts.HomeDir), opts.ProjectRoot)
...
func appendPluginHooks(out *[]ResolvedHook, nexuscodeHomeDir, projectRoot string) {
	if strings.TrimSpace(nexuscodeHomeDir) == "" {
		return
	}
	installed, _ := pluginpkg.LoadInstalled(nexuscodeHomeDir)
	for _, item := range installed {
		...
		for _, eventName := range events {
			...
			for _, h := range pkg.Manifest.Hooks[eventName] {
				...
				env := cloneEnv(h.Env)
				env["NEXUSCODE_PLUGIN_ROOT"] = pkg.Root
				env["NEXUSCODE_PLUGIN_NAME"] = item.Installed.Name
				env["NEXUSCODE_HOME"] = nexuscodeHomeDir
				env["NEXUSCODE_WORKSPACE_ROOT"] = projectRoot
				env["CLAUDE_PROJECT_DIR"] = projectRoot
				if item.Installed.Version != "" {
					env["NEXUSCODE_PLUGIN_VERSION"] = item.Installed.Version
				}
...
func nexuscodeHome(override string) string {
	if override != "" {
		return filepath.Join(override, SettingsDirname)
	}
	if dir := config.NexusCodeHomeDir(); dir != "" {
		return dir
	}
...
func legacyGlobalSettingsPath(homeDir string) string {
	dir := legacyNexusCodeHome(homeDir)
	if dir == "" {
		return ""
	}
...
func legacyNexusCodeHome(override string) string {
	if override != "" {
		return ""
	}
	...
	if sameCleanPath(legacy, nexuscodeHome("")) {
		return ""
	}
	return legacy
}
