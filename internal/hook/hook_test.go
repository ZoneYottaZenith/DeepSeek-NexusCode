...
func TestNormalizeCommandRepairsOnlyPowerShellFileEscapedQuotes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			command: `powershell -File \"C:\Users\Example\.nexuscode\hooks\archive-attachments.ps1\"`,
			want:    `powershell -File "C:\Users\Example\.nexuscode\hooks\archive-attachments.ps1"`,
		},
		{
			name:    "pwsh file path with spaces",
			command: `pwsh.exe -NoProfile -NonInteractive -File \"C:\Program Files\NexusCode Hooks\archive attachments.ps1\"`,
			want:    `pwsh.exe -NoProfile -NonInteractive -File "C:\Program Files\NexusCode Hooks\archive attachments.ps1"`,
		},
		{
			name:    "doubly escaped copied quotes",
			command: `pwsh -File \\\"C:\Program Files\NexusCode Hooks\archive attachments.ps1\\\" \"arg with spaces\"`,
			want:    `pwsh -File "C:\Program Files\NexusCode Hooks\archive attachments.ps1" "arg with spaces"`,
		},
		{
		...
		},
		{
			...
			command: `powershell -NoProfile -File "C:\Program Files\NexusCode Hooks\archive attachments.ps1"`,
			want:    `powershell -NoProfile -File "C:\Program Files\NexusCode Hooks\archive attachments.ps1"`,
		},
		{
...
func TestLoadNormalizesPowerShellFileEscapedQuotes(t *testing.T) {
	home := t.TempDir()
	bad := `powershell -File \"C:\Program Files\NexusCode Hooks\archive attachments.ps1\"`
	want := `powershell -File "C:\Program Files\NexusCode Hooks\archive attachments.ps1"`
...
func TestRepairablePowerShellFileArgs(t *testing.T) {
	command := `powershell -NoProfile -NonInteractive -File \"C:\Program Files\NexusCode Hooks\archive attachments.ps1\" -Mode \"startup\"`
	...
	wantArgs := []string{"-NoProfile", "-NonInteractive", "-File", `C:\Program Files\NexusCode Hooks\archive attachments.ps1`, "-Mode", "startup"}
...
func TestLoadIncludesPluginSessionStartHook(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	if got[0].Env["NEXUSCODE_PLUGIN_NAME"] != "superpowers" || got[0].Env["NEXUSCODE_WORKSPACE_ROOT"] != "/workspace" {
...
func TestLoadIncludesPluginClaudeCompatibilityHooks(t *testing.T) {
	home := t.TempDir()
	nexuscodeHome := filepath.Join(home, ".nexuscode")
	root := filepath.Join(nexuscodeHome, "plugins", "claude-pack")
	writeHookTestFile(t, filepath.Join(root, pluginpkg.CodexManifest), `{
  "name": "claude-pack",
  "version": "1.0.0",
  "skills": "skills"
  ...
  "hooks": {
	...
	if h := byEvent[PostToolUse]; h.Env["CLAUDE_PROJECT_DIR"] != "/workspace" || h.Env["NEXUSCODE_PLUGIN_NAME"] != "claude-pack" {
		t.Fatalf("plugin env = %#v", h.Env)
	}
}

func TestNexusCodeHomeOverridesGlobalHookPaths(t *testing.T) {
	home := t.TempDir()
	nexuscodeHome := filepath.Join(t.TempDir(), "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("NEXUSCODE_HOME", nexuscodeHome)
	if err := os.MkdirAll(nexuscodeHome, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(nexuscodeHome, SettingsFilename), []byte(`{"hooks":{"PostToolUse":[{"command":"echo rx"}]}}`), 0o644); err != nil {
		t.Fatal(err)
	}
	writeSettings(t, home, `{"hooks":{"PostToolUse":[{"command":"echo old"}]}}`)

	if got := GlobalSettingsPath(""); got != filepath.Join(nexuscodeHome, SettingsFilename) {
		t.Fatalf("GlobalSettingsPath = %q, want NexusCode home", got)
	}
	if got := TrustPath(""); got != filepath.Join(nexuscodeHome, TrustFilename) {
		t.Fatalf("TrustPath = %q, want NexusCode home", got)
	}
	hooks := Load(LoadOptions{})
	if len(hooks) != 1 || hooks[0].Command != "echo rx" {
		t.Fatalf("Load hooks = %+v, want NexusCode home hook only", hooks)
	}
}

func TestNexusCodeHomeDoesNotFallBackToLegacyWhenIsolated(t *testing.T) {
	home := t.TempDir()
	nexuscodeHome := filepath.Join(t.TempDir(), "rx-home")
	proj := t.TempDir()
	t.Setenv("HOME", home)
	...
	hooks := Load(LoadOptions{})
	if len(hooks) != 0 {
		t.Fatalf("Load hooks = %+v, want empty (isolated NEXUSCODE_HOME must not load legacy hooks)", hooks)
	}
	...
	if IsTrusted(proj, "") {
		t.Fatal("legacy trust must not be honored when NEXUSCODE_HOME is set and trust.json is absent")
	}
	if err := Trust(proj, ""); err != nil {
		t.Fatalf("Trust: %v", err)
	}
	if _, err := os.Stat(filepath.Join(nexuscodeHome, TrustFilename)); err != nil {
		t.Fatalf("Trust should write current NexusCode home trust file: %v", err)
	}
}
