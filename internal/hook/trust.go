...
// Trust gates project hooks. A project's .nexuscode/settings.json can run
...
// root. The trust flag lives in user-global state (<NexusCode home>/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (<NexusCode home>/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.nexuscode.
...
// TrustPath is <NexusCode home>/trust.json (homeDir overrides ~ for tests and
// legacy callers).
func TrustPath(homeDir string) string {
	return filepath.Join(nexuscodeHome(homeDir), TrustFilename)
}
