...
func TestRunProbesReportsMissingCommand(t *testing.T) {
	results := RunProbes(context.Background(), []string{"__nexuscode_missing_probe__ --version"})
	if len(results) != 1 {
...
func TestRunProbesAllowsStaticEnvAssignment(t *testing.T) {
	...
	results := RunProbesWithOverrides(context.Background(), []string{`NEXUSCODE_PROBE_ENV=ok envtool --version`}, map[string]string{"envtool": toolPath})
	if len(results) != 1 {
		t.Fatalf("results len = %d, want 1", len(results))
	}
...
func TestPrepareProbeCommandSetsCancellationBudget(t *testing.T) {
	cmd := exec.Command("nexuscode-test-probe")
...
func TestRunProbesFilterSubprocessEnv(t *testing.T) {
	...
	dir := t.TempDir()
	toolPath := filepath.Join(dir, "envtool")
	body := "#!/bin/sh\nprintf 'tok=%s' \"${NEXUSCODE_TEST_SECRET_TOKEN:-none}\"\n"
	if err := os.WriteFile(toolPath, []byte(body), 0o755); err != nil {
		t.Fatal(err)
	}
