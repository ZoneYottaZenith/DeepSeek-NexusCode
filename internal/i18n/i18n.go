...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — nexuscode setup
	StepSetKey      string // step 2 command label

	// `nexuscode init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // interactive session step desc
	StepRunDesc    string // nexuscode run step desc
	HelpFooter     string // dim footer linking to nexuscode help
	...
	ApprovalToolLabelConfigWrite           string // user-facing label for NexusCode-managed config write approvals
	...
	WroteFileFmt          string // "Wrote %s" — used for nexuscode.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from nexuscode.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: nexuscode run [--model NAME] <task>"
	...
	// `nexuscode upgrade` / `nexuscode update` — self-update
...
//
// Priority: override > NEXUSCODE_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"NEXUSCODE_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
