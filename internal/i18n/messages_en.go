...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `nexuscode`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold nexuscode.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `nexuscode`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `nexuscode setup`.",
	StepSetKeyHint: "run `nexuscode setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "nexuscode help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `nexuscode`",
	...
	PlanModeBashTrustReason:                "This bash command is not in NexusCode's built-in read-only set. Confirm only if this exact prefix is read-only for planning and research. Auto/YOLO approval cannot answer this trust prompt.",
	...
	ApprovalToolLabelConfigWrite:           "NexusCode config write",
	ConfigWriteSubjectPrefix:               "write NexusCode config: ",
	ConfigWriteReason:                      "This write targets a NexusCode-managed configuration file outside the workspace. It can change providers, sandbox rules, permissions, and MCP servers for future sessions, so it needs your explicit approval.",
	ConfigWriteDeclined:                    "the user declined this NexusCode config write - do not retry it; ask how they would like to proceed.",
	...
	OutputStyleHint:    "set agent.output_style in nexuscode.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in nexuscode.toml",
	...
	ArgLanguageAuto:     "auto-detect from NEXUSCODE_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .nexuscode/settings.json (project, after trust) or <NexusCode home>/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in nexuscode.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create NEXUSCODE.md in the project root",
	...
	MissingKeyIntro:       "nexuscode.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `nexuscode setup` or export DEEPSEEK_API_KEY=...), then run `nexuscode run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from nexuscode.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: nexuscode run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `nexuscode setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `nexuscode setup`.",
	...
	UsageBody: `nexuscode — a config- and plugin-driven coding agent (multi-model)

Usage:
  nexuscode [--model NAME] [-c|--continue] [--resume] [--copy] [--yolo] [--dir PATH]   interactive session (multi-turn; -c resumes the latest, --resume picks one, --copy continues in a duplicate)
  nexuscode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] [--copy] <task>   run one task and exit
  nexuscode review [--base BRANCH] [--commit SHA] [--model NAME]  AI-powered code review on local diffs
  nexuscode serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  serve over HTTP+SSE (with optional auth)
  nexuscode acp [--model NAME]                           serve Agent Client Protocol over stdio (also: nexuscode --acp)
  nexuscode setup [path]                                 interactive config wizard; writes nexuscode.toml (+ .env)
  nexuscode config auto-plan [off|on]                    configure automatic plan mode
  nexuscode config memory-v5 [off|observe|compact|on|status]  configure Memory v5
  nexuscode config reasoning-language [auto|zh|en]        configure visible reasoning language
  nexuscode mcp <add|remove|list|import>                 manage MCP servers in nexuscode.toml
  nexuscode init                                         show how to generate project memory (AGENTS.md)
  nexuscode doctor [--json]                              print redacted local diagnostics
  nexuscode doctor session <branch-id> [--zip] [--out PATH]  export a session conflict diagnostic zip
  nexuscode bot start|doctor|weixin-login                multi-channel IM bot gateway
  nexuscode upgrade [--check] [--force]                   self-update to the latest release (also: nexuscode update)
  nexuscode version
  nexuscode help
...
Examples:
  ...
  nexuscode run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | nexuscode run

Configuration:
  Resolution: flag > ./nexuscode.toml > <NexusCode home>/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'nexuscode setup' to scaffold a config; see docs/SPEC.md.
`,
}
