...
var ChineseTraditional = Messages{
	...
	SetKeyHint:      "設定好 API key 後執行 `nexuscode`。",
	ConfigLabel:     "設定",
	ModelsLabel:     "模型",
	...
	Ready:           "已就緒",
	GetStarted:      "開始使用",
	StepScaffold:    "生成 nexuscode.toml",
	StepSetKey:      "設定 API key",

	InitHint:       "專案記憶（AGENTS.md）在會話內由模型生成：執行 `nexuscode`，然後 `/init` —— 模型會分析程式碼庫並寫入。設定請用 `nexuscode setup`。",
	StepSetKeyHint: "執行 `nexuscode setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "互動式會話",
	StepRunDesc:    "執行單次任務",
	HelpFooter:     "nexuscode help · 檢視全部命令",

	ChatTip:           "對話上下文將跨輪保留。輸入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "沒有可恢復的會話 — 用 `nexuscode` 開一個新的",
	...
	PlanModeBashTrustReason:                "這條 bash 命令不在 NexusCode 內建唯讀集合中。只有在確認這個精確前綴用於計劃和研究時是唯讀的，才應核准。自動/YOLO 核准不能回答這個信任提示。",
	...
	ApprovalToolLabelConfigWrite:           "NexusCode 設定寫入核准",
	ConfigWriteSubjectPrefix:               "寫入 NexusCode 設定：",
	ConfigWriteReason:                      "這次寫入的目標是工作區之外的 NexusCode 託管設定檔。它可以改變後續工作階段的模型供應商、沙箱規則、權限和 MCP 伺服器，因此需要你的明確核准。",
	ConfigWriteDeclined:                    "使用者拒絕了這次 NexusCode 設定寫入；不要重試，請詢問使用者希望如何繼續。",
	...
	OutputStyleHint:    "在 nexuscode.toml 設定 agent.output_style 即可啟用（下次會話生效）",
	ThemeHeader:        "主題：",
	...
	SlashMCPNone:                 "沒有設定 MCP 伺服器 — 在 nexuscode.toml 加一個 [[plugins]] 條目",
	...
	ArgEffortMax:        "最高推理",
	ArgThemeCurrent:     "當前",
	ArgLanguageAuto:     "從 NEXUSCODE_LANG / 系統 locale 自動偵測",
	...
	ListHooksNone:       "無生效 hooks — 在 .nexuscode/settings.json（專案，需信任後）或 <NexusCode home>/settings.json（全域）設定",
	ListMcpHeader:       "MCP 伺服器",
	ListMcpNone:         "未連線 MCP 伺服器 — 在 nexuscode.toml（[[plugins]]）或專案 .mcp.json 中新增",

	MemoryNone:             "還沒有載入任何記憶 — 輸入 “/remember 內容” 可快速記錄，也可以在專案根目錄建立 NEXUSCODE.md",
	...
	MissingKeyIntro:       "nexuscode.toml 已設定好 — 只差一個 API key 就可以開始。",
	WroteFileFmt:          "已寫入 %s",
	SetupComplete:         "設定完成。",
	SetupCancelled:        "設定已取消。",
	TryHintFmt:            "試試: %s",
	NextHint:              "下一步：設定 API key（執行 `nexuscode setup` 或 export DEEPSEEK_API_KEY=...），然後執行 `nexuscode run \"你的任務\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳過 nexuscode.toml 裡的舊 %q 條目（指向 %s）— 請手動從 [[providers]] 裡刪除",
	...
	UsageRunHint:              "用法：nexuscode run [--model NAME] <task>",
	ErrorPrefix:               "錯誤：",
	...
	ProviderErrAuth:                "認證失敗 (HTTP 401)：API key 缺失、錯誤或已過期。請檢查 .env 中的金鑰，或執行 `nexuscode setup`。",
	...
	UsageBody: `nexuscode — 由設定和插件驅動的 coding agent（多模型）

用法：
  nexuscode [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   互動式會話（多輪；-c 恢復最近一次，--resume 選擇一個）
  nexuscode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   執行單次任務後退出
  nexuscode review [--base BRANCH] [--commit SHA] [--model NAME]  AI 程式碼審查（基於本機 diff）
  nexuscode serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  透過 HTTP+SSE 提供服務（支援可選認證）
  nexuscode acp [--model NAME]                           透過 stdio 提供 Agent Client Protocol（也可用：nexuscode --acp）
  nexuscode setup [path]                                 互動式設定精靈；生成 nexuscode.toml（及 .env）
  nexuscode config auto-plan [off|on]                    設定自動計畫模式
  nexuscode config memory-v5 [off|observe|compact|on|status]  設定 Memory v5
  nexuscode config reasoning-language [auto|zh|en]        設定可見思考語言
  nexuscode mcp <add|remove|list|import>                 管理 nexuscode.toml 裡的 MCP 伺服器
  nexuscode init                                         查看如何產生專案記憶（AGENTS.md）
  nexuscode doctor [--json]                              輸出脫敏的本機診斷資訊
  nexuscode doctor session <branch-id> [--zip] [--out PATH]  匯出會話衝突診斷 zip
  nexuscode bot start|doctor|weixin-login                多管道 IM bot 閘道
  nexuscode upgrade [--check] [--force]                   自更新至最新版本（也可用：nexuscode update）
  nexuscode version
  nexuscode help
...
範例：
  nexuscode
  nexuscode --continue
  nexuscode run "把 main.go 裡的 TODO 實現掉"
  nexuscode run --model mimo-pro "給這個函式補單元測試"
  echo "解釋這段程式碼" | nexuscode run

設定：
  優先順序：flag > ./nexuscode.toml > <NexusCode home>/config.toml > 內建預設值
  金鑰透過 api_key_env 從環境變數注入（如 DEEPSEEK_API_KEY）。
  執行 'nexuscode setup' 生成設定；詳見 docs/SPEC.md。
`,
	...
	GoalSetFmt:                 "目標已設定 → %s",
	GoalCleared:                "目標已清除",
	ProviderErrAuthRejected:    "認證失敗 (HTTP 401)：服務端拒絕了你的 API key。可能是 key 錯誤或已過期，也可能是服務端出現瞬時鑑權/額度問題——已退避重試仍失敗。請稍後再試，或檢查 .env 中的金鑰 / 執行 `nexuscode setup`。",
