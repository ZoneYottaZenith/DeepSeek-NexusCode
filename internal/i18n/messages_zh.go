...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `nexuscode`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `nexuscode`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `nexuscode setup`。",
	StepSetKeyHint: "运行 `nexuscode setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "nexuscode help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `nexuscode` 开一个新的",
	...
	PlanModeBashTrustReason:                "这条 bash 命令不在 NexusCode 内置只读集合中。只有在确认这个精确前缀用于计划和研究时是只读的，才应批准。自动/YOLO 审批不能回答这个信任提示。",
	...
	ApprovalToolLabelConfigWrite:           "NexusCode 配置写入审批",
	ConfigWriteSubjectPrefix:               "写入 NexusCode 配置：",
	ConfigWriteReason:                      "这次写入的目标是工作区之外的 NexusCode 托管配置文件。它可以改变后续会话的模型服务商、沙箱规则、权限和 MCP 服务器，因此需要你的明确批准。",
	ConfigWriteDeclined:                    "用户拒绝了这次 NexusCode 配置写入；不要重试，请询问用户希望如何继续。",
	...
	OutputStyleHint:    "在 nexuscode.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 nexuscode.toml 加一个 [[plugins]] 条目",
	...
	ArgEffortMax:        "最高推理",
	ArgThemeCurrent:     "当前",
	ArgLanguageAuto:     "从 NEXUSCODE_LANG / 系统 locale 自动检测",
	...
	ListHooksNone:       "无生效 hooks — 在 .nexuscode/settings.json（项目，需信任后）或 <NexusCode home>/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 nexuscode.toml（[[plugins]]）或项目 .mcp.json 中添加",

	MemoryNone:             "还没有加载任何记忆 — 输入 “/remember 内容” 可快速记录，也可以在项目根目录创建 NEXUSCODE.md",
	...
	MissingKeyIntro:       "nexuscode.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（运行 `nexuscode setup` 或 export DEEPSEEK_API_KEY=...），然后运行 `nexuscode run \"你的任务\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳过 nexuscode.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API key（缺失或未设置）。请在 .env 中配置密钥，或运行 `nexuscode setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `nexuscode setup`。",
	...
	UsageBody: `nexuscode — 由配置和插件驱动的 coding agent（多模型）

用法：
  nexuscode [--model NAME] [-c|--continue] [--resume] [--copy] [--yolo] [--dir PATH]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个，--copy 在副本中继续）
  nexuscode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] [--copy] <task>   执行单次任务后退出
  nexuscode review [--base BRANCH] [--commit SHA] [--model NAME]  AI 代码审查（基于本地 diff）
  nexuscode serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  通过 HTTP+SSE 提供服务（支持可选认证）
  nexuscode acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：nexuscode --acp）
  nexuscode setup [path]                                 交互式配置向导；生成 nexuscode.toml（及 .env）
  nexuscode config auto-plan [off|on]                    配置自动计划模式
  nexuscode config memory-v5 [off|observe|compact|on|status]  配置 Memory v5
  nexuscode config reasoning-language [auto|zh|en]        配置可见思考语言
  nexuscode mcp <add|remove|list|import>                 管理 nexuscode.toml 里的 MCP 服务器
  nexuscode init                                         查看如何生成项目记忆（AGENTS.md）
  nexuscode doctor [--json]                              输出脱敏的本地诊断信息
  nexuscode doctor session <branch-id> [--zip] [--out PATH]  导出会话冲突诊断 zip
  nexuscode bot start|doctor|weixin-login                多渠道 IM bot 网关
  nexuscode upgrade [--check] [--force]                   自更新到最新版本（也可用：nexuscode update）
  nexuscode version
  nexuscode help
...
示例：
  nexuscode
  nexuscode --continue
  nexuscode run "把 main.go 里的 TODO 实现掉"
  nexuscode run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | nexuscode run

配置：
  优先级：flag > ./nexuscode.toml > <NexusCode home>/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入（如 DEEPSEEK_API_KEY）。
  运行 'nexuscode setup' 生成配置；详见 docs/SPEC.md。
`,
}
