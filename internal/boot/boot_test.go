...
import (
	...
	"nexuscode/internal/agent/testutil"
	"nexuscode/internal/config"
	"nexuscode/internal/event"
	"nexuscode/internal/memory"
	"nexuscode/internal/netclient"
	"nexuscode/internal/plugin"
	"nexuscode/internal/provider"
	"nexuscode/internal/sandbox"
	"nexuscode/internal/tool"
	"nexuscode/internal/tool/builtin"

	// Blank import registers the provider kind the same way cmd/nexuscode's main
...
// cache-first wiring: a project NEXUSCODE.md is discovered at boot and folded
...
func TestBuildFoldsProjectMemoryIntoSystemPrompt(t *testing.T) {
	...
	writeFile(t, dir, "nexuscode.toml", `
...
model = "x"
api_key_env = "NEXUSCODE_TEST_KEY_UNSET"
`)
	writeFile(t, dir, "NEXUSCODE.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project NEXUSCODE.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering NEXUSCODE.md")
	}
}
...
model = "x"
`)
	writeFile(t, dir, ".nexuscode/skills/wskill.md",
		"---\ndescription: writer skill\nrunAs: subagent\nallowed-tools: bash, read_file, write_file\n---\nwriter body")
	writeFile(t, dir, ".nexuscode/skills/roskill.md",
...
// `nexuscode run`: a controller built via Build with NO SetSessionPath (exactly
...
api_key_env = "NEXUSCODE_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".nexuscode/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
[[plugins]]
name = "mockmcp"
command = "nexuscode-missing-mockmcp"
`)
	writeFile(t, dir, ".nexuscode/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
model = "x"
`)
	writeFile(t, dir, ".nexuscode/skills/readonlydig/SKILL.md", `---
...
func TestBuildTokenEconomyPlanModeCanConnectAllowedMCPSource(t *testing.T) {
	...
	setBootTokenProfileTestProvider(t, prov)
	writeFile(t, dir, "nexuscode.toml", fmt.Sprintf(`
default_model = "test-model"
...
func TestBuildOmitsExcludedSkillRootsFromPromptAndRuntimeList(t *testing.T) {
	...
	writeFile(t, home, ".nexuscode/skills/keep.md", "---\ndescription: keep\n---\nplaybook")
...
`)
	...
	// user-global NEXUSCODE.md in the real config dir could append; the test
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "nexuscode.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "nexuscode.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "nexuscode.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "nexuscode.toml"))
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "nexuscode.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "nexuscode.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "nexuscode.toml"))
...
// ~/.nexuscode/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	// migrated plugins.
	writeFile(t, proj, "nexuscode.toml", "")
	writeFile(t, filepath.Join(home, ".nexuscode"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".nexuscode", "sessions"), "chat-1.events.jsonl",
...
func TestBuildSkipsLegacySessionMigrationWhenIsolated(t *testing.T) {
	...
	nexuscodeHome := filepath.Join(home, "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	proj := robustTempDir(t)
	writeFile(t, proj, "nexuscode.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "nexuscode")
	...
	if _, err := os.Stat(filepath.Join(config.SessionDir(), "xdg-flat.jsonl")); !os.IsNotExist(err) {
		t.Fatal("legacy XDG flat session was imported but must not be when NEXUSCODE_HOME is set")
	}
	projectPath := filepath.Join(config.MemoryUserDir(), "projects", slug, "sessions", "project-chat.jsonl")
	if _, err := os.Stat(projectPath); !os.IsNotExist(err) {
		t.Fatal("legacy project session was imported but must not be when NEXUSCODE_HOME is set")
	}
}
...
[[plugins]]
name = "legacy-eager"
command = "nexuscode-missing-legacy-eager-mcp"
tier = "eager"
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "nexuscode.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
[[plugins]]
name = "legacy-lazy"
command = "nexuscode-missing-legacy-lazy-mcp"
tier = "lazy"
`)
...
func TestBuildDefaultsToNearestGitRoot(t *testing.T) {
	...
	writeFile(t, root, "nexuscode.toml", `
...
[[plugins]]
name = "slowserver"
command = "nexuscode-missing-slow-mcp-binary"
tier = "eager"
`)
