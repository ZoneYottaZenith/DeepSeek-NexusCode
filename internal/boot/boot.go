...
import (
	...
	"nexuscode/internal/environment"
	"nexuscode/internal/event"
	"nexuscode/internal/guardian"
	"nexuscode/internal/history"
	"nexuscode/internal/hook"
	"nexuscode/internal/installsource"
	"nexuscode/internal/instruction"
	"nexuscode/internal/jobs"
	"nexuscode/internal/lsp"
	"nexuscode/internal/memory"
	"nexuscode/internal/memorycompiler"
	"nexuscode/internal/migration"
	"nexuscode/internal/netclient"
	"nexuscode/internal/outputstyle"
	"nexuscode/internal/permission"
	"nexuscode/internal/planmode"
	...
	"nexuscode/internal/tool/sessiontool"
)
...
type Options struct {
	...
	// controller but are not persisted to nexuscode.toml.
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	// nexuscode.toml cannot override it), so concurrent workspaces agree.
	...
	if !ok {
		return nil, fmt.Errorf("%w %q (configured: %s); note: defining [[providers]] replaces the built-in presets, so add a [[providers]] entry for it or use a configured name, or run `nexuscode setup` to reconfigure", ErrUnknownModel, modelName, providerNames(cfg))
	}
	...
	if migErr != nil {
		sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "Config migration did not complete.", Detail: "config migration from ~/.nexuscode failed: " + migErr.Error()})
	...
	// Persistent memory (NEXUSCODE.md / AGENTS.md hierarchy + auto-memory index)
	...
	// managedConfig names the NexusCode-owned config FILES (config.toml,
	...
	managedConfig := builtin.NewManagedConfigPaths(config.NexusCodeManagedConfigPaths())
	...
	// The session-data guard blocks agent writes into NexusCode's own session
	...
	// resolves ordinary "ask" decisions to allow — preserving `nexuscode run`
	...
	skillRunner := func(sctx context.Context, sk skill.Skill, task string, runOpts skill.SubagentRunOptions) (string, error) {
		...
		if subagentStore == nil || parentSession == "" {
			// Headless runs (e.g. `nexuscode run`) have no persistent session to
	...
	// Custom slash commands (.nexuscode/commands + user dir). Best-effort: a malformed
	...
	executor := agent.New(execProv, reg, execSess, agent.Options{
		...
		UseMemoryCompilerLLMClassification: strings.TrimSpace(os.Getenv("NEXUSCODE_MEMORY_COMPILER_LLM_CLASSIFICATION")) == "true",
	}, sink)
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "nexuscode.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "nexuscode.toml" // match Config.Save() fallback
	}
	return path
}
...
// sessionGuard blocks writer-tool targets inside NexusCode's own session stores
// and makes bash warn when a command references them. managedConfig names the
// NexusCode-owned config files writable outside writeRoots after a fresh
