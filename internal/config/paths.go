...
func userConfigDir() string {
	return nexuscodeHomeDir()
}

func nexuscodeHomeDir() string {
	if dir := cleanEnvDir("NEXUSCODE_HOME"); dir != "" {
		return dir
	}
	if runtimeGOOS == "windows" {
		if dir := osUserConfigDir(); dir != "" {
			return filepath.Join(dir, "nexuscode")
		}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "nexuscode", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "nexuscode", "config.toml"))
	}
	return paths
}

func userSupportDir() string {
	if dir := cleanEnvDir("NEXUSCODE_STATE_HOME"); dir != "" {
		return dir
	}
	return nexuscodeHomeDir()
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "nexuscode")
	if current := nexuscodeHomeDir(); current != "" && samePath(path, current) {
		return ""
	}
	return path
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("NEXUSCODE_CACHE_HOME"); dir != "" {
		return dir
	}
	if dir := cleanEnvDir("NEXUSCODE_HOME"); dir != "" {
...
// IsolatedHomeDir returns the NEXUSCODE_HOME directory when it has been
...
func IsolatedHomeDir() string {
	return cleanEnvDir("NEXUSCODE_HOME")
}
...
// real location instead of a hardcoded ~/.nexuscode path.
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/nexuscode/config.toml"
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
...
// UserConfigPath is the user-global config.toml. It lives under NexusCode home:
// NEXUSCODE_HOME/config.toml, then ~/.nexuscode/config.toml on Unix-like systems,
// or %AppData%/nexuscode/config.toml on Windows. If %AppData% is unavailable on
// Windows, it falls back to %USERPROFILE%/AppData/Roaming/nexuscode/config.toml.
...
// from the current v1.8.1 NexusCode-home config path.
...
// NexusCodeManagedConfigPaths returns the NexusCode-owned user configuration
...
// TOML locations, and the legacy v0.x ~/.nexuscode/config.json. Individual
// files, never directories — the NexusCode home also holds credentials (.env),
// global hooks (settings.json), skills, and session stores, and none of those
// may ride along on a config repair.
func NexusCodeManagedConfigPaths() []string {
	var out []string
...
// NexusCodeHomeDir is the current NexusCode home directory. It honors
// NEXUSCODE_HOME, then uses ~/.nexuscode on macOS/Linux or %APPDATA%/nexuscode on
// Windows, with a %USERPROFILE%/AppData/Roaming fallback when %APPDATA% is
// unavailable.
func NexusCodeHomeDir() string { return nexuscodeHomeDir() }

// UserCredentialsPath is the nexuscode-owned global .env file under NexusCode
// home. It is the single source for provider credentials saved by NexusCode, so
// stale shell, Windows, project, or home env vars cannot silently override keys
// the user saved through setup or settings. "" when NexusCode home can't be
// resolved.
func UserCredentialsPath() string {
	dir := nexuscodeHomeDir()
	if dir == "" {
		return ""
	}
...
// Used by `nexuscode --continue` / `--resume` to find the recent ones. Empty
...
// MemoryUserDir returns the nexuscode user state root (…/nexuscode), under which
// the user-global NEXUSCODE.md and the per-project auto-memory store live. Empty
...
// commands), in canonical-first order. .nexuscode is ours; .agents / .agent /
...
// .nexuscode/settings.json (see internal/hook).
var ConventionDirs = []string{".nexuscode", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .nexuscode ends up the
...
// ~/.nexuscode/commands), the NexusCode home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .nexuscode/commands). Scanning the .claude /
...
func SourcePathForRoot(root string) string {
	root = resolveRoot(root)
	projectTOML := "nexuscode.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "nexuscode.toml")
	}
