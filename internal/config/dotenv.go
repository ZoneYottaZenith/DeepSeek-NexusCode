...
// loadDotEnv loads NexusCode's global .env for provider credentials. The
...
// expansion, then loads NexusCode's global .env for provider credentials.
...
// other and project files cannot redirect NexusCode's own config/credential
// paths.
...
func isProjectDotEnvControlKey(key string) bool {
	...
	if strings.HasPrefix(upper, "NEXUSCODE_") {
		return true
	}
	switch upper {
