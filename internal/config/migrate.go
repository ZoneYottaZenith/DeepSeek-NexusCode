...
// legacyConfig is the subset of the v0.x (~/.nexuscode/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.nexuscode/skills
...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to nexuscode's credentials store")
	}
...
// checks v1-era TOML first, then v0.5/v0.x ~/.nexuscode/config.json, and never
...
func MigrateLegacyIfNeededForRoot(root string) (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".nexuscode", "config.json")
	...
	if qqSecret := strings.TrimSpace(legacy.QQ.AppSecret); qqSecret != "" {
		...
		res.Warnings = append(res.Warnings, "your previous QQ Bot App Secret was saved to nexuscode's credentials store")
	}
...
func migrateMCPToUserConfig(projectRoots []string) (*MCPGlobalMigrationResult, error) {
	...
	for _, root := range normalizedMCPMigrationRoots(projectRoots) {
		addEntries(loadPluginEntriesFromTOML(filepath.Join(root, "nexuscode.toml")))
...
func legacyTOMLPaths(dest, home string) []string {
	...
	for _, legacy := range legacyXDGConfigPaths() {
		add(legacy)
		add(filepath.Join(filepath.Dir(legacy), "nexuscode.toml"))
	}
	add(filepath.Join(filepath.Dir(dest), "nexuscode.toml"))
	if home != "" {
		add(filepath.Join(home, ".nexuscode", "nexuscode.toml"))
	}
	return paths
}
...
// writeCredentialsEnv merges lines into NexusCode's global .env
// and pins them into the current process env so the just-built session resolves
// the key without a restart. Falls back to ~/.env only when NexusCode home can't
