...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. NexusCode reads
...
// stdio server; type/url/headers describe a remote one. NexusCode also accepts
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.nexuscode/config.json.
func legacyConfigPath() string {
	...
	return filepath.Join(home, ".nexuscode", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.nexuscode/config.json and returns its enabled
...
// already declare. nexuscode.toml's [[plugins]] win on a name collision: it is the
// NexusCode-specific, more explicit of the two, so it overrides the shared,
