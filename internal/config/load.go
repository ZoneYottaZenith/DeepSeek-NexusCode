...
// the v0.x ~/.nexuscode/config.json's mcpServers. Provider api_key_env values
// resolve from NexusCode's global .env, not from project .env files.
...
// each project's nexuscode.toml + .mcp.json are resolved independently without
// changing the process cwd, while provider keys stay rooted in NexusCode home.
func LoadForRoot(root string) (*Config, error) {
	...
	// project config's other fields, but do not let ./nexuscode.toml override
	...
	// nexuscode.toml must not be able to disable redaction or flip on the
	...
	// project nexuscode.toml doesn't drop the global config's MCP servers.
	...
	// nexuscode.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// ~/.nexuscode/config.json's mcpServers. Once the migration marker exists, the
...
// LoadForEdit returns a config to seed the `nexuscode setup` wizard when reconfiguring:
...
// of resetting to defaults. NexusCode's global .env is loaded so api_key_env
...
// MiMo providers needed by legacy refs that live outside nexuscode.toml, such as
