// Package config loads NexusCode's runtime configuration from TOML. Resolution order:
// flag > project ./nexuscode.toml > user config.toml (in the OS user-config dir) > built-in defaults.
...
// Config is NexusCode's runtime configuration.
type Config struct {
	ConfigVersion    int                 `toml:"config_version"`
	DefaultModel     string              `toml:"default_model"`
	Language         string              `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $NEXUSCODE_LANG
...
// setting: project nexuscode.toml values are ignored (see LoadForRoot), so a
...
type ServeConfig struct {
	...
	// Generate one with: nexuscode serve --hash-password --password '...'
...
// the project roots (.nexuscode/.agents/.agent/.claude under the workspace) and
...
type AgentConfig struct {
	...
	// PlannerModel enables two-model collaboration: the planner (a separate
	// session with read-only tools) analyzes the task and produces a plan, then
	// the executor (your default_model) carries it out. If you have both a fast
	// model (e.g. deepseek-v4-flash) and a capable model (e.g. deepseek-v4-pro)
	// configured, set planner_model to the pro model for deeper analysis while
	// the flash model handles fast execution — each in its own cache-stable
	// session. Empty disables two-model mode (single model does everything).
	...
	// .nexuscode/output-styles/<name>.md). Empty = the unmodified prompt.
...
// nexuscode.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
type PluginEntry struct {
	...
	// TrustedReadOnlyTools names raw MCP tool names that NexusCode should treat as
...
const DefaultSystemPrompt = `You are NexusCode, a coding agent focused on executing code tasks.
Built by ZoneYottaZenith.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `nexuscode run` autonomous (no TTY → ask
		// resolves to allow) while `nexuscode` prompts before writers. Users add
		...
		Bot: BotConfig{
			...
			Control:            BotControlConfig{Addr: "127.0.0.1:37913", TokenEnv: "NEXUSCODE_BOT_CONTROL_TOKEN"},
...
// not call this; loaded provider entries still resolve only from NexusCode's
// global .env.
