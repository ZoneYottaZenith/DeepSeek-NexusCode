...
// TestLoadDotEnvReadsGlobalCredentials proves `nexuscode setup`'s target — the
// nexuscode-owned credentials file under NexusCode home — is loaded from any
...
func TestLoadDotEnvDecodesGB18030Credentials(t *testing.T) {
	...
	t.Setenv("NEXUSCODE_HOME", filepath.Join(home, "nexuscode-home"))
	t.Setenv("NEXUSCODE_CREDENTIALS_STORE", "file")
	t.Setenv("PINNED_CN", "")
...
func TestDotEnvFileFilteredPreservesProjectScopeRules(t *testing.T) {
	...
	if _, ok := got["NEXUSCODE_HOME"]; ok {
		t.Fatalf("NEXUSCODE_HOME should be filtered: %+v", got)
	}
	if _, ok := got["HOME"]; ok {
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("NEXUSCODE_STATE_HOME", "")
	os.Unsetenv("NEXUSCODE_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("NEXUSCODE_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${NEXUSCODE_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("NEXUSCODE_STATE_HOME"); got != "" {
...
func TestLoadForRootResolvesProviderCredentialsOverInheritedEnv(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "nexuscode.toml"), []byte(`
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "NexusCode credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want NexusCode credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresNexusCodeStateHome(t *testing.T) {
	home := t.TempDir()
	state := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("NEXUSCODE_HOME", filepath.Join(home, "nexuscode-home"))
	t.Setenv("NEXUSCODE_STATE_HOME", state)

	want := filepath.Join(home, "nexuscode-home", ".env")
	...
	if strings.HasPrefix(UserCredentialsPath(), state) {
		t.Fatalf("credentials path must not live under NEXUSCODE_STATE_HOME: %q", UserCredentialsPath())
	}
}
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("NEXUSCODE_CREDENTIALS_STORE", "")
	os.Unsetenv("NEXUSCODE_CREDENTIALS_STORE")
	...
	if err := os.WriteFile(filepath.Join(project, "nexuscode.toml"), []byte(`credentials_store = "keyring"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms NexusCode-owned global
