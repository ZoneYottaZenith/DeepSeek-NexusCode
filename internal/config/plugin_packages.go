...
// in <NexusCode home>/plugin-packages.json so uninstall/disable can remove the
// entire bundle without editing user-authored config.
func mergeInstalledPluginPackages(cfg *Config, root string) []string {
	if cfg == nil {
		return nil
	}
	nexuscodeHome := NexusCodeHomeDir()
	if strings.TrimSpace(nexuscodeHome) == "" {
		return nil
	}
	installed, warnings := pluginpkg.LoadInstalled(nexuscodeHome)
...
func pluginPackageEnv(installed pluginpkg.InstalledPlugin, root string, env map[string]string) map[string]string {
	...
	out["NEXUSCODE_PLUGIN_ROOT"] = root
	out["NEXUSCODE_PLUGIN_NAME"] = installed.Name
	if installed.Version != "" {
		out["NEXUSCODE_PLUGIN_VERSION"] = installed.Version
	}
	return out
}
