...
// RenderTOML renders the config as annotated TOML in the `nexuscode setup` house style:
...
// nexuscode.toml stays focused on project behavior and intentionally excludes
...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# NexusCode configuration.\n")
	fmt.Fprintf(&b, "# Resolution order: flag > ./nexuscode.toml > %s > built-in defaults.\n", userConfigDisplayPath())
	b.WriteString("# Fields marked user/global only are not overridden by ./nexuscode.toml.\n")
	b.WriteString("# Secrets are named via api_key_env and stored in NexusCode's global .env; never put keys here.\n\n")

	fmt.Fprintf(&b, "config_version = %d   # schema marker for diagnostics; old versions may ignore it\n", configVersion(c))
	fmt.Fprintf(&b, "default_model = %q\n", c.DefaultModel)
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $NEXUSCODE_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $NEXUSCODE_LANG\n")
	}
	if scope != RenderScopeProject {
		fmt.Fprintf(&b, "credentials_store = %q   # legacy compatibility; provider keys are saved in NexusCode's global .env\n", normalizeCredentialsStore(c.CredentialsStore))
	}
	b.WriteString("\n")

	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; NEXUSCODE_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; NEXUSCODE_THEME_STYLE can override per run\n", style)
		} else {
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${NEXUSCODE_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if shouldRenderBot(c, defaults, scope) {
		...
		if strings.TrimSpace(c.Bot.Control.TokenEnv) != "" {
			...
			b.WriteString("# token_env = \"NEXUSCODE_BOT_CONTROL_TOKEN\"\n")
		}
	...
	if scope != RenderScopeProject {
		b.WriteString("[secrets]   # credential protection; user/global only, ./nexuscode.toml cannot override\n")
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"nexuscode-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `nexuscode setup`
