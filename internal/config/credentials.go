...
const (
	...
	credentialsKeyringService = "nexuscode"
	credentialClearedPrefix   = "# nexuscode-cleared "
)
...
// ResolveGlobalFirst resolves key from NexusCode's global .env only. Repeated
...
func credentialsStoreMode() string {
	if mode := strings.TrimSpace(os.Getenv("NEXUSCODE_CREDENTIALS_STORE")); mode != "" {
...
func loadCredentialStoreForRoot(root string) {
	...
	if p := UserCredentialsPath(); p != "" {
		loadDotEnvFileAs(p, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "NexusCode credentials (.env)"})
	}
}

// StoreCredentialLines stores KEY=value assignments in NexusCode's global .env
...
func pinCredentialAssignments(assignments map[string]string) {
	for key, value := range assignments {
		...
		recordCredentialSource(key, value, CredentialSource{Kind: CredentialSourceCredentials, Path: UserCredentialsPath(), Label: "NexusCode credentials (.env)"})
	}
}
...
func credentialSourceLabel(source CredentialSource) string {
	...
	case CredentialSourceCredentials:
		return "NexusCode credentials"
	case CredentialSourceHomeEnv:
		return "home .env"
	case CredentialSourceLegacy:
		return "legacy NexusCode credentials"
...
func storedCredentialValue(key string) (string, CredentialSource, bool) {
	if p := UserCredentialsPath(); p != "" {
		if value, ok := envFileValue(p, key); ok && value != "" {
			return value, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "NexusCode credentials (.env)"}, true
		}
	}
	return "", CredentialSource{}, false
}
