...
func legacyHome(t *testing.T) (src, dest, home string) {
	...
	return filepath.Join(home, ".nexuscode", "config.json"), userConfigPath(), home
}
...
func TestMigrateMCPToUserConfigOnUpgradeCollectsKnownSources(t *testing.T) {
	...
	writeLegacy(t, filepath.Join(filepath.Dir(dest), "nexuscode.toml"), `
[[plugins]]
name = "legacy-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(projectTOML, "nexuscode.toml"), []byte(`
[[plugins]]
name = "project-toml"
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(lateProject, "nexuscode.toml"), []byte(`
[[plugins]]
name = "late"
command = "late-bin"
`), 0o644); err != nil {
...
func TestMigrateImportsLegacyV1TOMLBeforeJSON(t *testing.T) {
	srcJSON, dest, _ := legacyHome(t)
	legacyTOML := filepath.Join(filepath.Dir(dest), "nexuscode.toml")
...
func TestMigrateImportsLegacyV1HomeTOMLBeforeJSON(t *testing.T) {
	...
	legacyTOML := filepath.Join(home, ".nexuscode", "nexuscode.toml")
	if err := os.MkdirAll(filepath.Dir(legacyTOML), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(legacyTOML, []byte(`
...
func TestMigrateImportsLegacyXDGConfigToPrimaryConfig(t *testing.T) {
	...
	legacy := filepath.Join(home, ".config", "nexuscode", "config.toml")
...
func TestMigrateImportsLegacyStateHomeDotEnvCredentials(t *testing.T) {
	...
	if strings.HasPrefix(currentCred, state) {
		t.Fatalf("current credentials path should not be under NEXUSCODE_STATE_HOME: %q", currentCred)
	}
	res, err := MigrateLegacyIfNeeded()
	if err != nil {
		t.Fatalf("migrate: %v", err)
...
func TestMigrateSupportData(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping since legacyOSSupportDir equals current nexuscodeHomeDir on Windows")
	}
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("NEXUSCODE_CREDENTIALS_STORE", "file")
