...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "nexuscode-plugin-example"}); err != nil {
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "nexuscode.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "nexuscode.toml")
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(root, "nexuscode.toml"), []byte(`
[[providers]]
name = "deepseek-flash"
kind = "openai"
base_url = "https://api.deepseek.com"
...
`), 0o644); err != nil {
	...
	projectPath := filepath.Join(root, "nexuscode.toml")
