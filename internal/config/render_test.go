...
func isolateUserConfigHome(t *testing.T) string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("NEXUSCODE_CREDENTIALS_STORE", "file")
...
func setRuntimeGOOS(t *testing.T, goos string) {
	t.Helper()
	t.Setenv("NEXUSCODE_TEST_GOOS", goos)
	old := runtimeGOOS
	runtimeGOOS = goos
	t.Cleanup(func() { runtimeGOOS = old })
}

func expectedDefaultNexusCodeHome(home string) string {
	if runtime.GOOS == "windows" {
		return filepath.Join(home, "AppData", "Roaming", "nexuscode")
	}
	return filepath.Join(home, ".nexuscode")
}
...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "nexuscode/config.toml") {
		t.Fatalf("display path = %q, want nexuscode/config.toml suffix", got)
	}
...
func TestUserConfigPathUsesNexusCodeHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	want := filepath.Join(expectedDefaultNexusCodeHome(home), "config.toml")
	if got := UserConfigPath(); filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("UserConfigPath() = %q, want %q", got, want)
	}
}

func TestNexusCodeManagedConfigPathsAreConfigFilesOnly(t *testing.T) {
	home := isolateUserConfigHome(t)
	...
	paths := NexusCodeManagedConfigPaths()
	for _, want := range []string{
		filepath.Join(home, "AppData", "Roaming", "nexuscode", "config.toml"),
		filepath.Join(home, ".nexuscode", "config.json"),
	} {
		found := false
	...
	// the sensitive NexusCode-home siblings (credentials, hooks, skills,
	...
	for _, got := range paths {
		...
		for _, forbidden := range []string{
			...
			NexusCodeHomeDir(),
			UserCredentialsPath(),
			filepath.Join(NexusCodeHomeDir(), "settings.json"),
			filepath.Join(NexusCodeHomeDir(), "skills"),
			filepath.Join(NexusCodeHomeDir(), "sessions"),
		} {
...
func TestUserConfigPathHonorsNexusCodeHome(t *testing.T) {
	home := isolateUserConfigHome(t)
	custom := filepath.Join(home, "custom-home")
	t.Setenv("NEXUSCODE_HOME", custom)
...
func TestLoadForRootUsesWindowsHomeFallbackWhenConfigDirUnavailable(t *testing.T) {
	...
	t.Setenv("NEXUSCODE_HOME", "")

	configPath := filepath.Join(home, "AppData", "Roaming", "nexuscode", "config.toml")
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${NEXUSCODE_PROXY_PASSWORD}",
		},
	}
	...
	orig.Bot.Feishu.OutboundMediaRoots = []string{"/tmp/nexuscode-media", "/srv/shots"}
	orig.Bot.Routes = []BotRouteConfig{{
		...
		WorkspaceRoot:    "/tmp/nexuscode-route",
	}}
	...
	orig.Bot.Connections = []BotConnectionConfig{{
		...
		WorkspaceRoot:    "/tmp/nexuscode-bot",
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "nexuscode-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/nexuscode-bot" {
	...
	if len(got.Bot.Feishu.OutboundMediaRoots) != 2 || got.Bot.Feishu.OutboundMediaRoots[0] != "/tmp/nexuscode-media" {
		t.Errorf("feishu outbound_media_roots not preserved: %+v", got.Bot.Feishu.OutboundMediaRoots)
	}
	if len(got.Bot.Routes) != 1 || got.Bot.Routes[0].WorkspaceRoot != "/tmp/nexuscode-route" || got.Bot.Routes[0].ChatID != "oc_group" {
	...
	if len(got.Bot.Connections[0].SessionMappings) != 1 || got.Bot.Connections[0].SessionMappings[0].Scope != "project" || got.Bot.Connections[0].SessionMappings[0].WorkspaceRoot != "/tmp/nexuscode-bot" {
...
func TestRenderTOMLRoundTripsProviderHeadersAndModelOverrides(t *testing.T) {
	...
	rendered := RenderTOML(orig)
	if !strings.Contains(rendered, `headers     = { HTTP-Referer = "https://app.example", X-Title = "NexusCode" }`) {
	...
	if p.Headers["HTTP-Referer"] != "https://app.example" || p.Headers["X-Title"] != "NexusCode" {
...
func TestRenderTOMLDefaultStepsDoNotOverrideGlobalConfig(t *testing.T) {
	...
	projectPath := filepath.Join(projectDir, "nexuscode.toml")
...
func TestIsolatedHomeDirReturnsCleanPath(t *testing.T) {
	raw := filepath.Join(t.TempDir(), "isolated-nexuscode")
	t.Setenv("NEXUSCODE_HOME", raw)
	got := IsolatedHomeDir()
...
func TestLegacyOSSupportDirEmptyWhenIsolated(t *testing.T) {
	isolateUserConfigHome(t)
	t.Setenv("NEXUSCODE_HOME", filepath.Join(t.TempDir(), "isolated-home"))
...
func TestCacheDirHonorsNexusCodeHome(t *testing.T) {
	home := t.TempDir()
	isolated := filepath.Join(home, "isolated-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("NEXUSCODE_HOME", isolated)
...
func TestCacheDirHonorsNexusCodeCacheHomeOverNexusCodeHome(t *testing.T) {
	home := t.TempDir()
	cacheHome := filepath.Join(home, "custom-cache")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv("NEXUSCODE_HOME", filepath.Join(home, "isolated-home"))
	t.Setenv("NEXUSCODE_CACHE_HOME", cacheHome)

	got := CacheDir()
	want := cacheHome
	if filepath.Clean(got) != filepath.Clean(want) {
		t.Fatalf("CacheDir() = %q, want %q (NEXUSCODE_CACHE_HOME must win)", got, want)
	}
}
...
func TestUserConfigLoadPathNoLegacyFallbackWhenIsolated(t *testing.T) {
	...
	productionHome := expectedDefaultNexusCodeHome(home)
...
func TestMigrateLegacyIfNeededSkipsWhenIsolated(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".nexuscode")
...
// security control: a cloned repository's nexuscode.toml must not be able to
...
func TestProjectConfigCannotOverrideSecrets(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "nexuscode.toml"), []byte(projectTOML), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !cfg.SecretsRedactToolOutput() {
		t.Error("project nexuscode.toml disabled redact_tool_output; [secrets] must stay user-global")
	}
	if cfg.Secrets.FilterSubprocessEnv {
		t.Error("project nexuscode.toml enabled filter_subprocess_env; [secrets] must stay user-global")
	}
	if cfg.Secrets.ProtectSensitiveFiles {
		t.Error("project nexuscode.toml enabled protect_sensitive_files; [secrets] must stay user-global")
	}
}
