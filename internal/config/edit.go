...
// separate from the `nexuscode setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to NEXUSCODE_LANG / locale.
...
// .mcp.json-only server would otherwise be serialized into nexuscode.toml or the
...
// and legacy JSON receives a disable marker for older NexusCode versions.
func RemovePluginFromSourcesForRoot(root, name string) (bool, error) {
	...
	if resolvedRoot != "." {
		projectTOML = filepath.Join(resolvedRoot, "nexuscode.toml")
	}
	isUserPath := false
	for _, path := range userPaths {
...
func pluginTOMLSourcePathForRoot(root, name string) string {
	projectTOML := "nexuscode.toml"
	if resolved := resolveRoot(root); resolved != "." {
		projectTOML = filepath.Join(resolved, "nexuscode.toml")
	}
...
// half-written nexuscode.toml that fails to parse on next load. Parent directories
// are created as needed.
//
// For project configs (./nexuscode.toml) the write is incremental: only sections
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# NexusCode project configuration.
...
// (SourcePath), or to ./nexuscode.toml when none exists yet — the conventional
...
// user's global config when root has no nexuscode.toml. Existing project files
