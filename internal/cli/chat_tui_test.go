...
func TestMain(m *testing.M) {
	...
	// the environment (NEXUSCODE_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func writeTUIImageCapabilityConfig(t *testing.T, root string) {
	...
	if err := cfg.SaveTo(filepath.Join(root, "nexuscode.toml")); err != nil {
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"nexuscode", "› /mcp"}
	...
	if !strings.Contains(lines[0], "nexuscode") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "nexuscode.toml")
...
func TestLanguageCommandAutoClearsLowerPriorityUserOverride(t *testing.T) {
	...
	if err := projectCfg.SaveTo("nexuscode.toml"); err != nil {
...
func TestSlashMigrateFromImportsExplicitSessions(t *testing.T) {
	...
	legacySessions := filepath.Join(home, "Old NexusCode", "sessions")
