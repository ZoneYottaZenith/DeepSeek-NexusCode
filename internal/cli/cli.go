// Package cli implements nexuscode's command-line entry: subcommand routing, flag
...
import (
	...
	"nexuscode/internal/agent"
	"nexuscode/internal/boot"
	"nexuscode/internal/config"
	"nexuscode/internal/control"
	"nexuscode/internal/event"
	fileencoding "nexuscode/internal/fileutil/encoding"
	"nexuscode/internal/i18n"
	"nexuscode/internal/notify"
	"nexuscode/internal/provider"
	"nexuscode/internal/provider/openai"
	"nexuscode/internal/sandbox"
	"nexuscode/internal/serve"
...
func Run(args []string, version string) int {
	cliVersion = version
	// Non-blocking npm version check (background goroutine).
	go checkNPMUpdate()
	...
	case "init":
		...
		// for config), so `nexuscode init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
...
func runAgent(args []string) int {
	...
	copySession := fs.Bool("copy", false, "with --resume/--continue: duplicate the session and continue in the copy (escape hatch when the original is held by another NexusCode process)")
...
func runServe(args []string) int {
	...
	*model = modelForResumePath(*model, *resume, cfg)
	...
	fmt.Printf("nexuscode serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string) int {
	fs := flag.NewFlagSet("nexuscode", flag.ContinueOnError)
	model := fs.String("model", "", "provider name (default: config default_model)")
	maxSteps := fs.Int("max-steps", 0, "max tool-call rounds (0 = use config/default)")
	cont := fs.Bool("continue", false, "resume the most recent saved session")
	fs.BoolVar(cont, "c", false, "shorthand for --continue")
	resume := fs.Bool("resume", false, "list saved sessions and pick one to resume")
	copySession := fs.Bool("copy", false, "with --resume/--continue: duplicate the selected session and continue in the copy (escape hatch when the original is held by another NexusCode process)")
	...
	*model = modelForResumePath(*model, resumePath, cfg)
	...
	// Proactive first-run detection: no API key configured anywhere means this
	// is almost certainly an unconfigured state, even when a config file exists
	// and boot.Build would nominally succeed. Run setup before the first build
	// attempt so the user gets guided configuration instead of a broken TUI.
	if isInteractive() && resumePath == "" && !hasProviderAPIKey(cfg) {
		fmt.Fprintln(os.Stderr, i18n.M.ReconfigureOnUnknownModel)
		if rc := interactiveSetup(defaultConfigTarget(), defaultEnvTarget()); rc != 0 {
			return rc
		}
		// Reload config so the setup changes are picked up
		cfg, _ = config.Load()
		*model = ""
	}
	...
	if err != nil && errors.Is(err, boot.ErrUnknownModel) && isInteractive() && !hasProviderAPIKey(cfg) {
		// Last-resort retry: boot.Build still failed after setup (e.g. a
		// non-interactive or resume path skipped the proactive check).
		fmt.Fprintln(os.Stderr, i18n.M.ReconfigureOnUnknownModel)
		if rc := interactiveSetup(defaultConfigTarget(), defaultEnvTarget()); rc != 0 {
			return rc
		}
...
// store. Keys always go to NexusCode's global .env so they
...
// project-local nexuscode.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "nexuscode.toml"
}

// defaultEnvTarget is the display target for the nexuscode-owned global
// NexusCode global .env.
func defaultEnvTarget() string {
	return config.CredentialsTargetDescription()
}

// resolveSetupTargets picks where `nexuscode setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./nexuscode.toml
...
// setupConfig runs the configuration wizard (the `nexuscode setup` command),
// writing config.toml to the user-global dir (or ./nexuscode.toml under --local)
// and API keys to NexusCode's global .env — never a project's own .env.
...
// initHint handles `nexuscode init`. Unlike a config scaffold, project memory is
...
// (and to `nexuscode setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to NexusCode's global .env. The wizard
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("nexuscode")),
		"",
...
// for NexusCode's global .env later and skips re-asking. A blank entry is fine —
...
// that older versions of the wizard wrote into nexuscode.toml. They collide
...
// can warn the user to clean up nexuscode.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `nexuscode setup`.
// The host-based slug also gives users a meaningful name to grep for in
// nexuscode.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing nexuscode.toml deal with SKU names
// directly.
...
// — the key step happens later in the wizard and NexusCode's global .env is updated then.
...
// NexusCode global .env. Re-pinning keeps hand-edited or previously saved values
...
// hasProviderAPIKey reports whether at least one configured provider has a
// non-empty API key in the environment. Used to distinguish a first-run/unconfigured
// state (no key anywhere) from a broken config (has keys but model mismatch).
// A nil or provider-less config returns false.
func hasProviderAPIKey(cfg *config.Config) bool {
	if cfg == nil {
		return false
	}
	for _, p := range cfg.Providers {
		if p.APIKeyEnv == "" {
			continue // provider without key reference can't be ready
		}
...
// are appended — so re-running `nexuscode setup` with a corrected key replaces the
...
func configReasoningLanguageCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./nexuscode.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
	...
	if !*local {
		...
		// --local writes ./nexuscode.toml and needs no user-config lock.
		unlock := config.LockUserConfigEdits()
		defer unlock()
	}
	cfg := config.LoadForEdit(path)
...
func configUsage() {
	fmt.Print(`Usage:
  nexuscode config auto-plan [off|on]
  nexuscode config memory-v5 [off|observe|compact|on|status]
  nexuscode config reasoning-language [--local] [auto|zh|en]
`)
}
