...
func TestGitStatusRenderWithinCompactsRepoBeforeBranch(t *testing.T) {
	status := gitStatus{Repo: "VeryLongDeepSeekNexusCodeWorkspace", Branch: "codex/cli-tui-status-row"}

	full := ansi.Strip(status.RenderWithin(80, statusAutoColor))
	if full != "VeryLongDeepSeekNexusCodeWorkspace@codex/cli-tui-status-row" {
...
func TestGitStatusRenderWithinKeepsDirtySuffix(t *testing.T) {
	status := gitStatus{
		Repo:      "VeryLongDeepSeekNexusCodeWorkspace",
...
func TestLoadGitStatus(t *testing.T) {
	...
	runGitForTest(t, root, "config", "user.email", "nexuscode@example.invalid")
	runGitForTest(t, root, "config", "user.name", "NexusCode Test")
