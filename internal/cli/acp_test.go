...
func TestACPInitializesWithoutAPIKey(t *testing.T) {
	...
	if !strings.Contains(out, `"protocolVersion":1`) || !strings.Contains(out, `"name":"nexuscode"`) {
...
func TestACPFactoryLoadsSessionCwdProjectConfig(t *testing.T) {
	...
	if _, err := config.SetCredential("NEXUSCODE_TEST_KEY", "test-key"); err != nil {
...
model = "fake-model"
api_key_env = "NEXUSCODE_TEST_KEY"
`), 0o644); err != nil {
		t.Fatal(err)
	}
	cmdDir := filepath.Join(project, ".nexuscode", "commands")
