...
// mcp.go holds the MCP server-management surface shared by the `nexuscode mcp`
...
// mcpCommand implements `nexuscode mcp <add|remove|list>`. It edits config only
...
func mcpGetCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexuscode mcp get <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexuscode mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to nexuscode.toml).

Usage:
  nexuscode mcp list
  nexuscode mcp get <name>
  nexuscode mcp add <name> <command> [args...]        stdio server
  nexuscode mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  nexuscode mcp add <name> --sse  <url>               remote (legacy SSE)
  nexuscode mcp import                                import MCP servers from cc-switch
  nexuscode mcp remove <name>
...
Examples:
  nexuscode mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  nexuscode mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
