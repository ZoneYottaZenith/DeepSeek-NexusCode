...
func TestMCPMigrationWaitsForCLIWorkspace(t *testing.T) {
	isolateCLIConfigHome(t)
	cwd := mustGetwd(t)
	if err := os.WriteFile(filepath.Join(cwd, "nexuscode.toml"), []byte(`
[[plugins]]
name = "cwd-project"
...
func TestMetadataCommandsDoNotProbeTerminalTheme(t *testing.T) {
	...
	if !strings.Contains(out, "nexuscode test-version") {
	...
	if !strings.Contains(out, "nexuscode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] [--copy] <task>") {
...
func TestRunNoArgsNonInteractivePrintsUsage(t *testing.T) {
	...
	if !strings.Contains(out, "nexuscode —") || !strings.Contains(out, "nexuscode run") {
...
func TestRunMigratesLegacyConfigBeforeConfigOnlyCommands(t *testing.T) {
	isolateCLIConfigHome(t)
	legacyPath := filepath.Join(filepath.Dir(config.UserConfigPath()), "nexuscode.toml")
...
func TestConfigAutoPlanLocalIsRejected(t *testing.T) {
	...
	if _, err := os.Stat("nexuscode.toml"); !os.IsNotExist(err) {
		t.Fatalf("nexuscode.toml should not be written, stat err=%v", err)
	}
...
func TestConfigAutoPlanIgnoresProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile("nexuscode.toml", []byte("[agent]\nauto_plan = \"on\"\n"), 0o644); err != nil {
...
func TestConfigReasoningLanguageLocalCreatesMinimalProjectOverride(t *testing.T) {
	...
	body, err := os.ReadFile("nexuscode.toml")
	if err != nil {
...
func TestFetchOrFallback(t *testing.T) {
	...
	t.Run("no key set returns static list (offline first-run)", func(t *testing.T) {
		t.Setenv("NEXUSCODE_FETCH_TEST_KEY", "")
		probe := config.ProviderEntry{
			BaseURL:   "http://127.0.0.1:1", // unreachable, no listener
			APIKeyEnv: "NEXUSCODE_FETCH_TEST_KEY",
...
// into nexuscode.toml. These collide with the wizard's own menu items, so
...
func TestWithBuiltinFamiliesDoesNotAddMissingMimo(t *testing.T) {
	// The user's case: a nexuscode.toml that defines only deepseek providers.
...
// with a single model). Re-running `nexuscode setup` must still surface the
...
func TestWriteDefaultConfigOmitsLegacyInternalMCPSections(t *testing.T) {
	path := filepath.Join(t.TempDir(), "nexuscode.toml")
