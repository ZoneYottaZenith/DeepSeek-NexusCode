...
func pluginUsage() {
  ...
  nexuscode plugin install <source> [--yes] [--dry-run] [--link] [--replace]
  nexuscode plugin list
  nexuscode plugin show <name>
  nexuscode plugin enable <name>
  nexuscode plugin disable <name>
  nexuscode plugin remove <name>
  nexuscode plugin doctor <name>`)
}
...
func pluginListCommand() int {
	st, err := pluginpkg.LoadState(config.NexusCodeHomeDir())
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
...
func pluginShowCommand(args []string) int {
	...
	root := pluginpkg.ResolveRoot(config.NexusCodeHomeDir(), p.Root)
...
func pluginSetEnabledCommand(args []string, enabled bool) int {
	...
	if err := pluginpkg.SetEnabled(config.NexusCodeHomeDir(), args[0], enabled); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
