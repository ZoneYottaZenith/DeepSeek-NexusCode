...
type chatTUI struct {
	...
	// "/mouse" or NEXUSCODE_DISABLE_MOUSE at startup; trades away in-app
	// drag-select, the transcript scrollbar, and wheel-scroll while it's on,
	// since the terminal no longer forwards those events to NexusCode.
	...
	// commands are custom slash commands loaded from .nexuscode/commands; each renders
...
func mouseCaptureOffByDefault() bool {
	v := strings.TrimSpace(os.Getenv("NEXUSCODE_DISABLE_MOUSE"))
	return v != "" && v != "0"
}
...
func (m chatTUI) update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		if !m.started {
			...
			// Check if there are user/assistant messages to replay (resume),
			// or only a system prompt (fresh session).
			hasHistory := false
			for _, m := range m.history {
			...
			if hasHistory {
				// Resume: skip ASCII art banner, show minimal info + history
				b.WriteString(accent("◆") + " " + bold("nexuscode") + "  " + dim("· "+m.label) + "\n")
				...
				// Fresh session: show full ASCII art banner
...
// toggleMouseCapture flips whether NexusCode owns the mouse. It's session-only
...
func (m *chatTUI) runExportCommand(input string) {
	...
	var b strings.Builder
	b.WriteString("# nexuscode session\n\n")
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	// ASCII art brand banner
	b.WriteString(banner())
	b.WriteString("\n")
	b.WriteString(accent("◆") + " " + bold("nexuscode") + "  " + dim("· "+label) + "\n")
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.nexuscode/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)
