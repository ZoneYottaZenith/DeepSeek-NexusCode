...
const (
	ghOwner        = "ZoneYottaZenith"
	ghRepo         = "DeepSeek-NexusCode"
...
// upgradeCommand handles `nexuscode upgrade` (and `nexuscode update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("nexuscode-%s-%s", runtime.GOOS, runtime.GOARCH)
	var asset *ghAsset
...
// "npm-v"). Prereleases are kept: only 1.x carries `nexuscode upgrade`, and the
...
func fetchLatestRelease(c *http.Client) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "nexuscode-cli")
...
// extractBinary pulls the "nexuscode" binary from a .tar.gz or .zip archive.
...
// rename it aside to .nexuscode.old first, then place the new binary.
