...
// sessionLeaseResumeRefusal is the startup-time refusal for `nexuscode
// [--resume|--continue]` and `nexuscode run --resume/--continue`: it names the
...
func sessionLeaseResumeRefusal(err error) string {
	return control.SessionInUseMessage(err) +
		"; close the other NexusCode window or process, or rerun with --copy to continue in a duplicated session"
}
