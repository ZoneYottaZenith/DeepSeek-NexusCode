...
func doctorRedactSessionsCommand(args []string) int {
	...
	if fs.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: nexuscode doctor redact-sessions [--dry-run] [--json] [--dir PATH]")
		return 2
	}
...
func doctorSessionCommand(args []string, version string) int {
	...
	for i := 0; i < len(args); i++ {
		...
		switch arg {
		case "-h", "--help":
			fmt.Fprintln(os.Stdout, "usage: nexuscode doctor session <branch-id-or-path> [--zip] [--out PATH]")
			fmt.Fprintln(os.Stdout, "")
			fmt.Fprintln(os.Stdout, "Bundles the session transcript, persistence sidecars, conflict diagnostics,")
			fmt.Fprintln(os.Stdout, "and the recovery parent chain into a zip for support. Unlike `nexuscode doctor`,")
		...
		default:
			...
			if ref != "" {
				fmt.Fprintln(os.Stderr, "usage: nexuscode doctor session <branch-id-or-path> [--zip] [--out PATH]")
				return 2
			}
			ref = arg
		}
