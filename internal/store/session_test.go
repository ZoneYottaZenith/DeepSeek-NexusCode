...
func TestSessionSidecarLayout(t *testing.T) {
	const p = "/home/u/.nexuscode/sessions/abc.jsonl"
	cases := []struct {
		name string
		got  string
		want string
		...
		{"goal-state", SessionGoalState(p), "/home/u/.nexuscode/sessions/abc.goal-state.json"},
		{"event-log", SessionEventLog(p), "/home/u/.nexuscode/sessions/abc.events.jsonl"},
		{"event-index", SessionEventIndex(p), "/home/u/.nexuscode/sessions/abc.event-index.json"},
		{"conflict-log", SessionConflictLog(p), "/home/u/.nexuscode/sessions/abc.conflicts.jsonl"},
		{"lock", SessionLockFile(p), p + ".lock"},
		{"lease-lock", SessionLeaseLock(p), p + ".lease.lock"},
		{"lease-info", SessionLeaseInfo(p), p + ".lease.json"},
		{"checkpoint", SessionCheckpointDir(p), "/home/u/.nexuscode/sessions/abc.ckpt"},
		{"jobs", SessionJobsDir(p), "/home/u/.nexuscode/sessions/abc.jobs"},
		{"cleanup-pending", SessionCleanupPending(p), "/home/u/.nexuscode/sessions/abc.cleanup-pending.json"},
	}
	for _, c := range cases {
		if c.got != c.want {
...
func TestSessionSidecarFiles(t *testing.T) {
	...
	want := []string{
		p + ".meta",
		"/home/u/.nexuscode/sessions/abc.goal-state.json",
		"/home/u/.nexuscode/sessions/abc.events.jsonl",
		"/home/u/.nexuscode/sessions/abc.event-index.json",
		"/home/u/.nexuscode/sessions/abc.conflicts.jsonl",
	}
	if len(got) != len(want) {
